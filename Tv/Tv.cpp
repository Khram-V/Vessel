...
{ static char *S,Name[]="\n>Tv «BName»"
     ...
     "\n «-bgi=2-8»   ©""1991-2016 V.Khramushin";
  if( Ac<2 )Break( Name );              //
  for( int i=1; i<Ac; i++ )             // Выборка последнего имени
  if( Av[i][0]!='-' && Av[i][0]!='/' )  // в списке параметров
  if( (S=strrchr( strcpy( Name,Av[i] ),'.' ))!=0 ){ S[4]=0; strlwr( S+1 );
    if( !strcmp( S+1,"inf" )
     || !strcmp( S+1,"int" ) )Inf_View( Name );   else
    if( !strcmp( S+1,"grd" ) )GRD_View( Name );   else
    if( !strcmp( S+1,"pcx" ) )PCX_View( Name );   else
    if( !strcmp( S+1,"chr" ) )FNT_View( Name,2 ); else
    if( !strcmp( S+1,"vct" ) )FNT_View( Name,3 ); else
    if( !strcmp( S+1,"vfn" ) )FNT_View( Name,4 ); else
    if( !strcmp( S+1,"dc2" )
     || !strcmp( S+1,"dc3" ) )DCM_View( Name,5 ); else
    if( !strcmp( S+1,"dw2" ) )DCM_View( Name,6 ); else
    if( !strcmp( S+1,"bln" ) )DCM_View( Name,7 ); else
    if( !strcmp( S+1,"nc"  ) )Net_CDF( Name );
    else Break( "? %s не тот",Name ); return 0;
  } return 2;
}
//
...
//
static void PCX_View( char *Name )
{
 FILE *Fs;
