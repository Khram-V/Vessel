...
Model& Model::cubic( bool painting_Model, _Real Alfa )
...
#define Vertex( k )color( c[k],0,Alfa ),dot( out( V[k] ) )
...
Model& Model::dipole(_Real s,bool paint,_Real Alfa)// s: внутри -1<0<+∞ снаружи
  ...
  for( int i=1-ns; i<ns-1; i++ ){ glBegin( GL_QUAD_STRIP );        // параллели
    for( int j=0;  j<=rs;  j++ ){ Real t=(j%rs)*(-2.0*M_PI)/rs;    // меридианы
      for( int k=0; k<=1;  k++ ){ W=Meridian[abs(i+k)];            // закраска
        ...
        color( C[(i+k+ns)*cp/ns/2],0,Alfa );
        dot( out( (Vector){W.x,W.y*cos(t),W.y*sin(t)} ) ); // C[(ns-i-k)*(cp-1)/ns/2] );
      }
    } glEnd();
  } return *this;
}
#if 1
Model& Model::sphere( int ns,int rs, bool paint, _Real Alfa )
  ...
  for( int i=0; i<ns; i++ ){ glBegin( GL_QUAD_STRIP );     // rs>=3 - меридианы
    for( int j=0; j<=rs; j++ ){ Real t=(j%rs)*(2.0*M_PI)/rs;
      for( int k=0; k<=1; k++ ){ Real s=M_PI*( 0.5-Real( i+k )/ns );
        color( C[(ns-i-k)*(cp-1)/ns],0,Alfa );             // интервалы цветов+1
        dot( out( (Vector){ sin(s),cos(s)*cos(t),cos(s)*sin(t) } ) );
    } } glEnd();
  } return *this;
}
Model& Model::torus( _Real r,_Real R, int ns,int rs, bool paint, _Real Alfa )
  ...
  for( int i=0; i<ns; i++ ){ glBegin( GL_QUAD_STRIP ); // glColor3ubv(C[ns-i-1]);
    for( int j=0; j<=rs; j++ ){ Real t=M_PI*Real(j)*2/rs;
      ...
      //glNormal3dv( Model.out((Vector){sin(s),cos(s)*cos(t),cos(s)*sin(t)}) );
        color( C[(ns-i-k)*(cp-1)/ns],0,Alfa );              // интервалы цветов
