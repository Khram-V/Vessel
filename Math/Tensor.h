...
//operator Real(){ return (x*y)%z; }          //... определитель — объем триады
  ...
  Vector operator * ( _Matrix,Vector );  // переход из Л(В)СК в А(Г)СК
  Vector operator * ( Vector,_Matrix ); //  проекции на базисные оси
...
// Tensor& operator=( _Real m ){ init( m ); return det(); }
 Tensor& operator=( _Matrix m ); // { x=m.x,y=m.y,z=m.z; return det(); }
  ...
  //
 Vector AtL( _Vector A ) const; // { return A=A/Tensor( *this ); }
 Vector LtA( _Vector A ) const; // { return A=Matrix( *this )*A; }
...
//friend Vector operator / ( Vector,_Tensor );
  //
