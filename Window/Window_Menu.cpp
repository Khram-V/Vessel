...
}                                   // все расчёты с представлением результатов
bool TextMenu::Draw()            // картинка со списком строк запросов/форматов
{ Activate(); { TextContext Ts;  // здесь предусматривается изменение масштабов
    Area( X,Y,Lx+2,Ly+2 ).Activate( true );               // активация разметки
    glScalef( Fnt->W,-Fnt->H,1.0 ),glTranslatef(0,-Ly-2,0); // листа для текста
...
{ if( !Up )return Place::Mouse( x,y ); x/=Fnt->W,y=(y-8)/Fnt->H+1;
  for( int k=0; k<Num; k++ )
