...
{ Place *Up;                 // адрес вверх в последовательном списке наложений
  ...
  Font   *Ft;                // ссылка на шрифт из эпохи СССР или chr:DesignCAD
  unsigned *Img;             // временное хранилище для растрового отображения
  int  MouseState,xo,yo,     // внутренний статус и учётное местоположение мыши
...
public:                      // открытая информация и прочие внешние процедуры
  ...
  friend class glContext;    // временная активация графического поля openGL
  //
...
//
  void String( const char* );                        // с разделённым
  void String( const char *T, Real x, Real y );      //  позиционированием
  void String( Course,const Real*,const char* );     //+ горизонтальные надписи
public:                                    // выбор текстового прямоугольника
  Real AlfaHeight(),AlfaWidth();           // условная высота и ширина буквочки
  SIZE AlfaRect( const char* Msg );        // c кодировками UTF8 всегда и везде
  Place& AlfaBit( const byte *CCCP=NULL ); // OEM сходу включён _8x08_8x14_8x16
...
explicit glContext(const Window*); // конструктор=пролог графического конвейера
        ~glContext();          // деструктор=эпилог - возврат к былой зарисовке
};
#endif
...
extern
 const unsigned char _8x08[],_8x14[],_8x16[]; // растровые шрифты из эпохи СССР
#endif
