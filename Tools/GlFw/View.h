...
#include "../../Math/Volume.h"       // базовые структуры тензорной математики
...
inline aR point( aR a ){ glVertex3dv( a ); return a; }   // контекстная точка  aR point( aR,colors );                                 // с установкой цвета
  aR points( aR,_Real Size,colors=empty );               // • завершенная точка  aR line( aR,aR );                                      // завершённая прямая  aR line( aR,aR,colors );                               // с конкретным цветом
...
virtual bool Mouse( int b,int x,int y ); // курсор-указатель - мышь свободная
protected:
