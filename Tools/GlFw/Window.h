...
//void close();
  ...
  //
  void PutChar( unsigned short Key );                 // один символ --> кольцевой буфер
 ...
 virtual bool KeyBoard( unsigned short key )
      { if( extKey )if( extKey( key ) ){ Draw(); return true; } return false; }
  Window& KeyBoard( bool(*inKey )( unsigned short )=NULL ){ extKey=inKey; }
...
private:
 struct{ byte Code,Key; } KeyBuffer[0x40]; // накопительный буфер от клавиатуры
  ...
  bool(*extKey)(unsigned short); // свободная процедура обработки прерываний клавиатуры
