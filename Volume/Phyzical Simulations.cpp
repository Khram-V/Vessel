...
Real Particle_viscosity=1.0,     // коэффициент сопротивления для шара[0.2-0.4]
...
static int  Draw_flow = 1;       // 0-нет; 1-вызванное; 2-полное; 3-вместе
static Real Dipole_offset=0.0,   // скоростная деформация диполя
            Particle_volume=1.0, // контрольный объем моделируемой частицы
            Video_size=1.0,      // масштабный коэффициент прорисовки корпускул
            fx=0.2,fy=0.2,fz=0.0;// пространственный щаг векторных разметок
static Vector Mc={ 0,0,0 };      // текущий сдвиг центра масс всех корпускул
...
//
  ...
  color( black ),Win.Print( 1,-2,"%+d = \n",kT ),
  color( blue ),Win.Print( "%s-%s( %0.3gx%-0.3g ){ %d } by %s - vsc( %3.1f ) ... ",
      Gravity_mode?"Gravity":"Flying",
      (const char*[]){ "*","Куб","Тор","Шар","Диполь"}[MTyp],
      Particle_volume,Video_size,Fly.nc,linear_Model?"solid":"edges",
      Particle_viscosity );
  if( Draw_flow && Fly.nc>0 )
  { Real Sc=hypot( fx,fy );//*_Pd;              // масштаб отрисовки векторов
  ...
  //if( MTyp==Sphere || MTyp==Dipole )Mxl*=EqSphere; else (Tensor)Mxl/=2.0;
    ...
    { Point P={ x,y,z }; P+=Mc; int k;                // точка в сеточном узле
      if( Draw_flow&1 )
      { //int k=0; Vector V=Zero;
//#pragma omp parallel for private ( k,V ) //reduction(+ : V.x,V.y,V.z )
        for( k=0,V=Zero; k<Fly.nc; k++ )
        { Ball &B=Fly.Dip[k];
          V+=dipole_v( B.M*EqSphere,P-B.R );          // вызванные скорости
        }
        arrow( P-V*Sc,P+V*Sc,0.25,lightblue );
      }
      if( Draw_flow&2 )
      { //int k=0; Vector V=Zero;
//#pragma omp parallel for private ( k,V )
        ...
        //V+=(Vector){Fly.MV*EqSphere*Fly.nc};
        arrow( P-V*Sc,P+V*Sc,0.125,lightgreen );
    } }
  } // Win.Show();
//
...
//#pragma omp parallel for     // OpenGL такого не позволяет
    for( int j=0; j<Fly.nc; j++ )
    { Ball &B=Fly.Dip[j];
      Mxl.set( B.M );
      Point R=B.R; Mxl=R;
      Model_Draw( MTyp );
      Mc+=(Vector){ R.X,R.Y,R.Z};
      if( Fly.lt>0 )
      { //glPointSize( 6 ); glBegin( GL_POINTS ); dot( R,red ); glEnd();
        //glPointSize( 1 );
        spot( R,6,red );
  ...
  axis( Win,Screen_Size/2,Screen_Size/2,Screen_Size/3,"X","Y","Z",cyan );
...
#pragma omp parallel for
  ...
  if( !(++kT%50) )Win.Draw(); return false;
}
...
{ switch( key )
  { case _Esc: exit( 0 );                         // выход из программы
    ...
    case 'V': Particle_viscosity+=0.2; break;
    case 'v': Particle_viscosity-=0.2; break;   // трение о воздух
   ...
   default :
    { fixed Typ=MTyp,Sts=Win.ScanStatus();
      ...
      { case ' ':
        ...
        case _PgUp: Video_size*=1.1; Typ=255; break;
        case _PgDn: Video_size/=1.1; Typ=255; break;
       default:
        ...
        { if( Fly.nc )Fly.Clear(),(Point)Mxl=(Point){0,0,0}; else
          { Fly.Create( key=='='?729                        // остановка кубика
            ...
            Video_size=pow( 0.9,21 );                      // 21 отсчёт малости
...
//            r — радиус малой образующей окружности тора
  switch( typ )
  { case Cube  : Mxl*=pow( Video_size/(Real)Mxl,0.333 );
                 Mxl.cubic( painting ); break;
    case Sphere: Mxl*=pow( Video_size*0.75/_Pi/(Real)Mxl,0.333);
                 Mxl.sphere( 15,24,painting ); break;
    case Dipole: Mxl*=pow( Video_size*0.75/_Pi/(Real)Mxl,0.333);
                 Mxl.dipole( Dipole_offset,painting ); break;
    case Torus:{ const Real K=0.01,r=1.0;       // коэффициент к диаметру дырки
                 Mxl*=pow( Video_size*_Pi/_Pi/2/(1+K)/(Real)Mxl,0.333 )/2;
