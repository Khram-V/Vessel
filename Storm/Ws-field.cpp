...
#pragma omp parallel for
      ...
      Vessel->Floating()     // наложение уровня + расчёт воздействия на корпус
             .Moving( Ts );  // моделирование гидродинамики корабля на волнении
...
Waves&
Waves::Solving( _Real Tr, _Real Ts, _Vector dR )   // Время, шаг времени и хода
{ //
...
//
Waves& Waves::FreeSurface( _Real Ts )    // + поле скорости задается в реальном
{ Real Sigma=_Pd*Cw/Length,              // = волновая частота          [1/сек]       dTs=Cw*Ts/Ds/Sigma;               //   масштабе подвижных частиц [м/сек]
#pragma omp parallel for
  for( int x=1; x<Mx; x++ )
  for( int y=1; y<My; y++ )
...
( _Real Tr, _Real Ts, _Real Step )        // Обход по периметру акватории для
{ Real Bh=0,B1=H[0][1].z,Dh,Dx,Dy;        // пере-экстраполяции граничных точек
  ...
  { int Ys=y>0?-1:0,Yn=y<My?1:0,jy=Ys+Yn, // над градиентом волнового склона
    ...
    Dh=4.0*( Wave( Tr+Ts/8,y,x ).z-Wave( Tr-Ts/8,y,x ).z );              // [м]
    Dx=( H[y][Xe+x].z-H[y][x+Xw].z )/( Ds*(Xe-Xw) ) - Tensor::x.x*Dh*Step/Ts;
    Dy=( H[Yn+y][x].z-H[y+Ys][x].z )/( Ds*(Yn-Ys) ) - Tensor::x.y*Dh*Step/Ts;
...
//
Waves& Waves::ThreeSurface( _Real Ts )   // + поле скорости задается в реальном
{ Real Sigma=_Pd*Cw/Length,              // = волновая частота          [1/сек]
         dTs=Cw*Ts/Ds/Sigma;             //   масштабе подвижных частиц [м/сек]
#pragma omp parallel for
  for( int y=1; y<My; y++ )
...
( _Real Tr, _Real Ts, _Real Step )        // Проход по периметру всей акватории
{ Vector Bh={0,0,0},B1=H[0][1],Dh,Dx,Dy;  // пере-экстраполяция граничных точек
  ...
  Real kW=Cw*Ts*tK/Ds/(tK+1);   // скорость волны от скорости пересечения сетки
...
//   || (j==0 && x.y<=0.0) || (j==My && x.y>=0.0) )
    { Dh = ( Wave(Tr+Ts/6,j,i)-Wave(Tr-Ts/6,j,i) )*3.0; // входящая волна [м]
      Dx -= Dh*(x.x*Step/Ts);                           // прямая корректировка
      Dy -= Dh*(x.y*Step/Ts);                           // по скорости хода
