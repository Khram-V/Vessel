...
{ Vector l=Vessel->Locate,w,r; l.z=R.z;    // сводится к повороту в плоскости Z
         iWave( T*Cw-Diagonal,r=AtL( R+l ),w );
  return LtA( r )-l;
}
...
{ Vector W=Wind.Wave( T,R )+Swell.Wave( T,R )+Surge.Wave( T,R )-2*R; return W;
}
...
{ Real Bh=0,B1=H[0][1].z,Dh,Dx,Dy;        // пере-экстраполяции граничных точек
  ...
  { int Ys=y>0?-1:0,Yn=y<My?1:0,jy=Ys+Yn, // над градиентом волнового склона
    ...
    Dx=( H[y][Xe+x].z-H[y][x+Xw].z )/( Ds*(Xe-Xw) ) - Matrix::x.x*Dh*Step/S;
    Dy=( H[Yn+y][x].z-H[y+Ys][x].z )/( Ds*(Yn-Ys) ) - Matrix::x.y*Dh*Step/S;
...
//   || (j==0 && x.y<=0.0) || (j==My && x.y>=0.0) )
    { Dh = 3.0*( Wave(Tr+S/6,j,i)-Wave(Tr-S/6,j,i) );  // входящая волна [м]
      Dx -= (x.x*Step/S)*Dh;                           // прямая корректировка
      Dy -= (x.y*Step/S)*Dh;                           // по скорости хода
...
{ Real Rx=Mx/2+A.x/Ds,                  // выборка индексов и отступов от них
  ...
  return (1.0-Rx)*( (1-Ry)*H[y][x]+Ry*H[y+1][x] )      // простые разности
            + Rx *( (1-Ry)*H[y][x+1]+Ry*H[y+1][x+1] ); // билинейных пересчётов
}
