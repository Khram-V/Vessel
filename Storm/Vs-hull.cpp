...
//
static int kl=0,kr=0;           // левые и правые индексы обрабатываемой шпации
...
{ static int s=-1,k=0; int i,j;          // не предполагается повторного чтения
  if( s!=sp ){ s=sp; kr=k=0; }            // выбор нового левого шпангоута sp
  if( !s )                                // левее других шпангоутов больше нет
  { if( k<Frame[0].length )                 // на последовательное перечисление
    { R=Frame[0][m=kr=k++]; return true; }  // пробная точка правее ахтерштевня
  } else
  if( s<Nframes )                           // иначе выбор вправо внутри шпации
  { Flex &G=Frame[s],&L=Frame[s-1];
    ...
    { _Vector S=Stem[i];
      ...
      if( S==G[k]  ){ Sm[i]=2; break; } // в приоритете на раз точка шпангоута
      if( S==L[kl] )j=i; else;
      if( Keel[s-1]<=S.x && S.x<=Keel[s] )  //!<= шпации с накрытием шпангоутов
      { if( !k ){ if( S.z<=G[0].z && S.y<G[0].y ||    // точки под форштевнем и
                 S.z<=G[0].z && S.z>=L[kl].z )j=i; } else // под днищем корпуса
        if( k>=G.length ){ if( S.z==G[k-1].z && S.y>G[k-1].y ||  // над верхней
                          S.z>=G[k-1].z || S.z>=L[kl].z )j=i; } else // палубой
        if( S.z>=G[k-1].z && S.z<=G[k].z && S.z>=L[kl].z )j=i; }     // внутри
      if( j>=0 ){R=S; m=StemPost|j; return Sm[j]=S.x<Keel[s]?1:2; } //= штевень
    } if( k<G.length ){ R=G[k]; m=kr=k++; return true; }//номер точки шпангоута
  ...
  { Flex &G=Frame[s-1];
    ...
    { R=G[kr=k=G.length-1]; m=LeftFrame|k++; return true; }// транцевая затычка
...
{ static int s=-1,k=0;         // одноразовый подход целиком ко всему шпангоуту
  if( s!=sp ){ s=sp; kl=k=0; } // фиксируется правый шпангоут, левый - в поиске
  ...
  { if( k<Frame[s-1].length )  // обычная поисковая фиксация ++ по перечислению
      { L=Frame[s-1][kl=k]; m=LeftFrame|k++; return true; }
  } else
  if( s>0 )                          // вариант со средними шпациями-трапециями
  { Flex &G=Frame[s-1],&R=Frame[s];
...
//    if( S==G[k] ){ k++; j=i; } else
      ...
      { if( !k ){ if( S.z<=G[0].z && S.y<=G[0].y
                   || S.z<=G[0].z && S.z>=R[kr].z )j=i; } else
        if( k>=G.length ){ if( S.z==G[k-1].z && S.y>G[k-1].y ||
                               S.z>=G[k-1].z || S.z>=R[kr].z )j=i; } else
        if( S.z>=G[k-1].z && S.z<=G[k].z && S.z>=R[kr].z )j=i; }
      if( j>=0 ){ L=S; m=SternPost|j; return St[j]=S.x<Keel[s]?1:2; }
    } if( k<G.length ){ m=LeftFrame|k; kl=k; L=G[k++]; return true; }
  ...
  { Flex &G=Frame[0];
    ...
    if( !Stern.length && k<G.length ){ m=kl=k=G.length-1; L=G[k++]; return true; }
  } return false;
}
...
Ok: Str=stringData( Fh );            // Длинная строка в буфере входного файла
  ...
  { Vector V; Real x,y,z;            // здесь продолжается серия проверок
    ...
    //
/*  for( k=Stem.length-1; k<=0; k-- )
    { Vector &S=Stem[k];
      for( n=0; n<Nframes; n++ )
      { //Flex &F=Frame[n];
        for( i=0; i<Frame[n].length; i++ )
         if( Frame[n][i]==S )Frame[n].Delete( i );
      }
    }
*/  //    Чтение последовательного потока завершено, но если смещена основная
 ...
 // Shell=(unsigned**)Allocate( Nframes+3,24*sizeof(unsigned) );  //! двумерный
    ...
    { Flex &S = !n?Stern:Stem;                  // указываются транцевые
#if 0
     int Previous=-1;
...
#define ENV { VL=R-Ll; VR=Rr-L; RL=R-L; if( fb ){ VL.y/=3,VR.y/=3,RL.y/=3; } \
     ...
     Vector L,Ll,LS=Zero,ls=Zero,VL,NL,NR,RL,  // первая нормаль указывает вниз
      ...
      kl=0,kr=0;                                     // индексы активной шпации
...
#else
 static struct InList            // индексные списки кривых последовательностей
