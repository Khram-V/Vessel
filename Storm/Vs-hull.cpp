...
//         { for( k=l; k>=j; k-- ){ V=S[k]; V.y=0; F.Insert( m )=V; } break; }
    ...
    for( n=1; n<=Nframes; n++ )
    { Vector V=Frame[n][0];  if( V.y ){ V.y=0; Frame[n]/=V; }
...
Reto_Deck:
      ...
      { W = R[sr+1]-L[sl];                  // на выходе строго sl==wl и sr==wr
        V = R[sr]-L[sl+1];
//      if( norm( W ) < norm( V ) )
        if( dir( W ).x > dir( V ).x )
            Ins( n ) = rf[++sr]; else
...
#if 0                                         /// !!! не отлажено - не работает
      while( sl<L.len-1 )if( L[sl+1]==L[sl] )++sl; else break; wl=sl+1;
      while( sr<R.len-1 )if( R[sr+1]==R[sr] )++sr; else break; wr=sr+1;
      //  V = dir( R[wr]-L[wl] );       // стрингер-ориентир в основании поиска
      if( wl<L.len || wr<R.len )
      { for( int nl=0,nr=0; (!nl || !nr) && nl<5 && nr<5; )
        { if( nr>0 )nr++; else if( wr>=R.len-1 )++nr,wr=R.len; else
          { W=R[wr+1]-R[wr];
            if( !W || ( dir( W )%dir( R[wr]-R[wr-1] ) )<0.95
                   ||   dir( R[wr+1]-L[wl] ).x<0.95 )nr=1; else wr++;
          }
          if( nl>0 )nl++; else if( wl>=L.len-1 )++nl,wl=L.len; else
          { W=L[wl+1]-L[wl];
            if( !W || ( dir( W )%dir( L[wl]-L[wl-1] ) )<0.95
                   ||   dir( R[wr]-L[wl+1] ).x<0.95 )nl=1; else wl++;
          }
        } goto Reto_Deck;
      }
#else
...
//          lfr = W.z<-V.z;                   /// ???
       ...
       Rst: if( r>0 )
          ...
          //
     const Real S20=0.015; //0.3420201433;
          W=R[zr]-L[zl];         // исправление индекса с нарушением отсчётчика
...
//        if( sqr( W.z )<sqr( V.z ) )
//        if( fabs( W.z )< fabs( V.z ) )
//        if( W.z<-V.z )
          if( W.x>V.x )
              Ins( n )=rf[++sr]; else                     // или по её проекции
              Ins( n )=lf[++sl];
#else
