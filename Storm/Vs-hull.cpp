...
{ int i,j,k,m,n; Vector V; //,W;        // S+=S[0] точка последняя <- начало кольца
  if( !Y.len )return;               // штевни остаются как есть, без расширений
  if( !S.len ){ Y[0].x=X; S+=Y[0]; } //  одна новая точка ставится по шпангоуту
#define inx( x,s ) ( (St && (s<=x))||(!St && (x<=s)) )  // за штевнем [s<=x<=s]
#define oux( x,s ) ( (St && (x<s) )||(!St && (s<x) ) )  // вне штевня  [x<s<x]
  //
  //   для начала расставляются абсциссы на неоднозначных транцевых расширениях
  //      по самым дальним точкам на контурах штевней в диаметральной плоскости
  //       попутно отмечаются крайние индексы использованных на штевнях абсцисс
  //
  for( m=n=-1,k=0; k<Y.len; k++ )               // крайние точки получаются m<n
  if( S.len==1 ){ Y[k].x=S[0].x,m=0,n=1; } else // здесь одна абсцисса для всех
  ...
  { Real x,mx; V=Y[k];               // корректируются точки транцевых контуров
    ...
    { x = Inter( V.z,S[i].z,S[i+1].z,S[i].x,S[i+1].x ); // x - идёт вне штевней
      if( j<0 ){ j=i; mx=x; } else if( inx( mx,x ) )mx=x,j=i;
    }
    if( j<0 )Y.Delete( k ); else// точка расширения вне диаметральной плоскости
    { Y[k].x=mx; if( m<0 )n=(m=j)+1; else if( j<m )m=j; else if( j>=n )n=j+1; }
  }
...
//{static int i=23;
//cprint( 2,i,"Альянс-%d - основной = m=%d < %d=n",++i-23,m,n ); //getch();
//}
...
//    if( intor( Y[k].y,V.y,Y[k+1].y ) ) // равные ординаты у вертикальной стенки
//{
//printf( "k=%d, eps<%.3g = { %.3g,%.3g,%.3g } \n",k,abs( V-Y[k] ),V.x,V.y,V.z );
      if( abs( V-Y[k] )>eps )Y.Insert( k+1 )=V;           // предустанавливался
//}
    }
    //!   переброс векторной кривой с возвратом ординатной сборки на штевни
    //
    for( i=m-1; i>=0; i-- )Y/=S[i];
    for( i=n; i<S.len; i++ )Y+=S[i];
    for( S.len=i=0; i<Y.len; i++ )S+=Y[i];
  }
  for( i=1,k=0; i<S.len; i++ ) // совпадающие точки в подводной части снимаются
  { if( S[k]!=S[i] || S[k].z>0.0 )k++; if( k!=i )S[k]=S[i]; } S.len=k;
...
Ok: Str=stringData( Fh );            // Длинная строка в буфере входного файла
  if( *Str++==30 ){                  //  '\30'=''
   ...
   Real x,y,z; int i,j,k,l,m,n;      // ... точки по циклам двойной вложенности
    ...
    //
    lfr=false; // выбор левой стороны от штевня со смежными правыми шпангоутами
    do
   ...
   uList rf,lf;                       // индексы доступа к точкам
    ...
    Reto_Deck:
      ...
      { //       предварительный просмотр поверхности с поиском бортовых сломов
        ...
        //
       const Real S12=0.2079116908,C12=0.9781476007;
...
//          lfr = norm(W.z,W.x)<norm(V.z,V.x);// длинная диагональ вправо-вверх
//          lfr = V.x>W.x;
            lfr = norm( R[zr+1]-L[zl] )<norm( R[zr]-L[zl+1] );
//          lfr = fabs( ( R[zr+1]-L[zl] ).z )<fabs( ( R[zr]-L[zl+1] ).z );
//          lfr = R[zr+1].z<L[zl+1].z;
            if( lfr )goto Rst;                //...подтягивается к правой точке
        Lst:if( l>0 )
            { if( ++zl>=wl-1 )zl=wl,l=0; else
              if( dir(L[zl+1]-L[zl])%dir(L[zl]-L[zl-1])<C12 || L[zl+1]==L[zl] )
              { l=0; if( r>0 )r=2; } else if( l>=0 )--l;              ///=0 ???
              continue;
            }
            if( lfr )continue;
        Rst:if( r>0 )
          ...
          if( ( W=dir( W ) ).z>S12 ^ R[zr+1].z<R[zr].z )
          for( i=zr-1;i>sr;i--){ if( R[i].z-L[zl].z>=0)zr=i; else break; } else
          if( W.z<-S12 ^ L[zl+1].z>L[zl].z  )
          for( i=zl-1;i>sl;i--){ if( R[zr].z-L[i].z<=0)zl=i; else break; }
        }
...
/* C06=0.9945218954,C08=0.9902680687,C12=0.9781476007,C18=0.9510565163,
   ...
   C84=0.1045284633,C86=0.06975647374 ...
   cos(78°)=sin(12°) ==> S06=C84,S12=C78,S08=C82,S04=C86 ... */
