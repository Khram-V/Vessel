...
//  for( V=S[j],k=0; k<Y.len-1; k++ ) // последовательно по транцевым ординатам
    ...
    for( S.len=i=0; i<Y.len; i++ )S+=Y[i];     // переброс результата на штевни
  }
...
//    if( j==l )goto Rep; else
      { //
       ...
       Flex &F=lfr?Frame[n]:Frame[n+1];          // фиксация ссылки на шпангоут
        if( S[l].z>=F[-1].z )for( k=j; k<=l; k++ )F+=S[k]; else
   ...
   uList rf,lf;                       // индексы доступа к точкам
    ...
    Reto_Deck:
      ...
      { //       предварительный просмотр поверхности с поиском бортовых сломов
        ...
        //
       const Real C12=0.9781476007,S20=0.3420201433;
...
#if 1
        { W=R[sr+1]-L[sl];                 // обработка перпендикулярных
          V=R[sr]-L[sl+1];                 // пересечений под кормовым подзором
...
//        if( norm( W ) < norm( V ) )Ins( n )=rf[++sr];  // по длине гипотенузы
          if( dir( W ).x>dir( V ).x )Ins( n )=rf[++sr];  // или по её проекции
                               else  Ins( n )=lf[++sl];
#else
