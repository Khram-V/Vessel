...
static Vector Inter( _Real x, _Real x0,_Real x1, _Vector v0,_Vector v1 )
...
//    if( A==B && A.z<0.0 )F.Delete( k ); else              // без совпадений и
      ...
      { if( Stm )B=e5( Inter( 0.0,A.z,B.z,A,B ) ); else
        if( A.y || B.y ){ B.y=e5( Inter( 0.0,A.z,B.z,A.y,B.y ) ); B.z=0.0; }
        else continue;
        F.Insert( k+1 )=B;
} } } }
#if 1
...
#define oux( x,s ) ( (St && (x<s) )||(!St && (s<x) ) )  // вне штевня  [x<s<x]
  ...
  if( S.len==2 ){ Y[k].x=Inter( Y[k].z,S[0].z,S[1].z,S[0].x,S[1].x ),m=0,n=2; }
  else
  ...
  { V=Y[0];         // одна точка означает одинаковое утолщение по всему штевню
    ...
    for( i=0; i<S.len; i++ ){ S[i].y=V.y;       // для всех одинаковая ордината
  ...
  //     расстановка точек штевней по неоднозначным кривым транцевых расширений
  {
    for( V=Y[0],i=0; i<S.len-1; i++ )// подкильный участок транцевых расширений
     if( intoi( S[i+1].z,V.z,S[i].z ) // здесь полагаются все ординаты нулевыми
      && intoi( S[i+1].x,V.x,S[i].x ) )
    { //if( !V.y )--i;
      for( j=i; j>=0; j-- )Y/=S[j]; break;
    }
    for( V=Y[m=Y.len-1],j=S.len-1; j>i; j-- )        // надпалубный фрагмент в
     if( intoi( S[j].z,V.z,S[j-1].z )                // диаметральной плоскости
      && intoi( S[j].x,V.x,S[j-1].x ) )
    { //if( !V.y )j++;
      for( ; j<S.len; j++ )Y+=S[j]; break;
    }
    //!   основной блок формирование контуров штевней с транцевыми расширениями
#if 1
    for( k=i; k<m; k++ )      // k: [i..m]
    for( j=0; j<S.len; j++ )  // расстановка интерполированных точек из штевней
    { V=S[j];                 //  n - ограничивает выборку козырьком фальшборта
      for( i=j+1; i<S.len; i++ ) // отвод индексов к максимуму внешней абсциссы
       if( S[i].z<=V.z && inx( S[i-1].x,S[i].x ) )j=i; V=S[j];    ///??? скачок
...
#else
    //                          поиск верхнего слома на штевне для переключения
    //                         к простой старой последовательности для углового
    for( n=1; n<S.len; n++ )   // отсечения на палубе в диаметральной плоскости
     if( St && S[n].z>=0 && S[n].z<=S[n-1].z && S[n].x>S[n-1].x )break; else
     if( !St && S[n].z>=0 && S[n].z<=S[n-1].z && S[n].x<S[n-1].x )break;
//n=min( n+2,S.len ); ///???
    // одна из точек обязана попадать точно в угол четырехугольника {x,z}
    //         на кривой ординат, то есть, под его самое первое касание
    //         или подводное утолщение штевня по нижней ординате транца
    for( k=i+1; k<m; k++ )      // k: [i..m]
    for( j=0; j<=n; j++ )     // расстановка интерполированных точек из штевней
    { V=S[j];                 //  n - ограничивает выборку козырьком фальшборта
      for( i=j+1; i<=n; i++ ) //  отводом индексов к максимуму внешней абсциссы
       if( S[i].z<=V.z && inx( S[i-1].x,S[i].x ) )j=i; V=S[j];    ///??? скачок
      if( intoi( Y[k-1].z,V.z,Y[k].z ) )           // по неоднозначным функциям
      { V.y=Inter( V.z,Y[k-1].z,Y[k].z,Y[k-1].y,Y[k].y ); // транцевых контуров
        if( Y[k]!=V && Y[k-1]!=V ){ Y.Insert( k++ )=V; ++m; } // ещё дополнение
    } }
    //!  в заключение прицепляются неоднозначные надпалубные обгрызки надстроек
    //                   начиная с последовательности отсчетов от Y[m] и S[n+1]
    V=Y[--m];        // существующая опорная точка штевня по транцу S[n]=Y[m-1]
    while( true )
    { i=min( S.len-1,n+1 ); if( n>=i )n=i;              // два прострела вперёд
      j=min( Y.len-1,m+1 ); if( m>=j )m=j;              //              и вверх
      if( i>n && j>m )                                         /// i=>n<, k=>m<
      { if( fabs( Y[j].z-V.z )<fabs( S[i].z-V.z ) )            //?    кто ближе
        { Y[j].x=Inter( Y[j].z,S[i].z,V.z,S[i].x,V.x ); } else
        { S[i].y=Inter( S[i].z,Y[j].z,V.z,Y[j].y,V.y ); Y.Insert(j)=S[i]; n++;}
      } else if( i>n ){ S[i].y=Y[j].y; Y.Insert( j )=S[i]; n++; }
        else if( j>m ){ Y[j].x=S[i].x; }
        else break; V=Y[j]; ++m;
    }
#endif
    //!   переброс векторной кривой с возвратом ординатной сборки на штевни
    //
...
Ok: Str=stringData( Fh );            // Длинная строка в буфере входного файла
  if( *Str++==30 ){                  //  '\30'=''
   Flex C,L,R; Vector V,W; bool lfr; // векторы шпангоутных контуров по шпациям
   Real x,y,z; int i,j,k,l,m,n;      // ... точки по циклам двойной вложенности
...
#else
    //           Ахтерштевень
    //                      (начало считывания последовательного потока данных)
    ...
    //                         Форштевень - отсчет абсцисс шпангоутов и штевней
    //                                      ведется от кормового перпендикуляра
    ...
    //
    lfr=false;  // выбор стороны для форштевеня со смежными правыми шпангоутами
    do
    for( n=1; n<Nframes; n++ )
    {    i=j=l=0;
    ...
    Rep:                     // выборка цельного фрагмента штевня внутри шпации
...
#if 1
                // пусть аргумент фрагмента будет меняться только слева направо
     Flex &F=lfr?Frame[n]:Frame[n+1]; int f;      // фиксация ссылки шпангоутов
      for( m=S[j].x<=S[l].x,C.len=0,k=j;k<=l;k++)if(m^lfr)C+=S[k];else C/=S[k];
      for( k=0; k<C.len; k++ ) // противоположные в шпации шпангоуты отсекаются
       if( !lfr && C[k].x==Keel[n+1] || lfr && C[k].x==Keel[n] )C.Delete( k );
      if( !C.len )goto Rep;
      //
      //  самая простая привязка контуров штевней по наклонам ветвей шпангоутов
      //
      (W=C[-1]).y=0; x=norm( F[f=0]-W );    // фрагмент контура приводится к ДП
      for( k=1; k<F.len; k++ ){ if( (y=norm( F[k]-W ) )<=x )x=y,f=k; } // поиск
      if( !f )m=0; else if( f>=F.len-1 )m=1; else    // особый случай замыкания
           m=F[f-1].y>=F[f].y && F[f].y>=F[f+1].y;   // ниже под развалом борта
      if( !m )                                 // подкильный фрагмент форштевня
      { for( k=C.len-1; k>=0; k-- )if( F[f]!=C[k] )F.Insert( f )=C[k];
      } else                                  // затем участки завала форштевня
        for( k=0; k<C.len; k++ )if( F[f]!=C[k] )F.Insert( f+1 )=C[k];
#else
     ...
     Real zu,zd,z0,z1;
      ...
      //
      z0=Frame[n][0].z; z1=Frame[n+1][0].z;
      if( zd<=z0 || zd<=z1 )                                   // килевые точки
      { if( z0!=z1 )li=z0>z1; F=&Frame[n+!li]; x=Keel[n+li];   //  у шпангоутов
        if( lfr^li )
        { for( m=j; m<=l; m++ )if( S[m].x!=x )(*F)/=S[m]; } else
        { for( m=l; m>=j; m-- )if( S[m].x!=x )(*F)/=S[m]; }
        if( i<S.len )goto Rep; else continue;
      }
      //              и по верхним палубам всё иначе - только внутренний контур
      //
      z0=Frame[n][-1].z; z1=Frame[n+1][-1].z;             // фрагмент с верхней
      if( zu>=z0 || zu>=z1 )                              // точкой выше палубы
      { if( z0!=z1 )li=z0<z1; F=&Frame[n+!li]; x=Keel[n+li];      // привязка к
        ...
        if( i<S.len )goto Rep; else continue;             // к следующей шпации
      }
      //    внутри контура шпангоута (здесь требуется тщательная отработка >=<)
      //                                      отсечка по корме и в сторону носа
//    if( lfr ){ while( S[j].x==Keel[n+1] )++j; while( S[l].x==Keel[n+1] )--l; }
//        else { while( S[j].x==Keel[n] )++j; while( S[l].x==Keel[n] )--l; }
      for( F=&Frame[n+!lfr],k=1; k<F->len; k++ )     // сначала выбор шпангоута
       if( intoi( (*F)[k-1].z,(zu+zd)/2,(*F)[k].z ) ) // снизу - вверх
//       { for( m=j; m<=l; m++,k++)if( (*F)[k]!=S[m] )F->Insert(k)=S[m]; break;
   ...
   uList rf,lf;                       // индексы доступа к точкам
    ...
    { L.len=R.len=lf.len=rf.len=0;    // начальные точки совпадают, и ненулевые
      ...
      for( x=Keel[n-1],i=0; i<L.len; i++ )//L[i].x=x;
       if( L[i].x>x )L[i].y=0.0,L[i].x=x; //else if( L[i].z>0.0 )L[i].y/=3.0;
      for( x=Keel[n],i=0; i<R.len; i++ )//R[i].x=x;
       if( R[i].x<x )R[i].y=0.0,R[i].x=x; //else if( R[i].z>0.0 )R[i].y/=3.0;
      //
     ...
     int sl=1,sr=1;           // поиски по контурам шпангоутов от третьих точек
      //
      ...
      { if( norm( R[1]-L[0] )>=norm( R[0]-L[1] ) ) // плоскости (временно-пока)
        { Ins( n )=lf[1]; Ins( n )=rf[1];
          for( i=2; i<L.len; i++ )
           if( norm(R[1]-L[i])<norm(R[1]-L[i-1]) )Ins(n)=lf[sl=i]; else break;
        } else
        { Ins( n )=rf[1]; Ins( n )=lf[1];
          for( i=2; i<R.len; i++ )
           if( norm(R[i]-L[1])<norm(R[i-1]-L[1]) )Ins(n)=rf[sr=i]; else break;
     ...
     int wl,wr,zl=sl,zr=sr;           // индексные отсчеты по уровню ватерлинии
      for( wl=L.len-1; wl>sl; wl-- )if( L[wl].z<=0.0 ){ zl=wl; break; }
      for( wr=R.len-1; wr>sr; wr-- )if( R[wr].z<=0.0 ){ zr=wr; break; }
      //
    ...
    Reto_Deck:
      ...
      { //       предварительный просмотр поверхности с поиском бортовых сломов
        ...
        //
       const Real S08=0.139173101,S12=0.2079116908,C12=0.9781476007;
        //
        //   как бы полупроход с поиском сломов и совпадающих точек
        if( sl==zl && sr==zr )                // обход начальных предустановок
        { int l=wl-zl+1,r=wr-zr+1;            // поиске всего от киля до палубы
          ...
          { if( zl>=wl-2 || zr>=wr-2 )        // палуба перекрывается без каких
            ...
            lfr = fabs( W.z )<fabs( V.z );    // длинная диагональ вправо-вверх
            if( lfr )goto Rst;                //...подтягивается к правой точке
        Lst:if( l>0 )
            { if( ++zl>=wl-1 )zl=wl,l=0; else
              if( dir(L[zl+1]-L[zl])%dir(L[zl]-L[zl-1])<C12 || L[zl+1]==L[zl] )
                { if( r>0 )r=3; l=0; } else if( l>0 )--l; continue;
            }
            if( lfr )continue;
        Rst:if( r>0 )
            { if( ++zr>=wr-1 )zr=wr,r=0; else // на точку ниже последнего слома
              if( dir(R[zr+1]-R[zr])%dir(R[zr]-R[zr-1])<C12 || R[zr+1]==R[zr] )
                { if( l>0 )l=3; r=0; } else if( r>0 )--r; continue;
          ...
          //
          W=dir( R[zr]-L[zl] );         //if(0)if( W.x<C12 || fabs( W.z )>S08 )
          if( W.z>S12 )for( i=zr-1; i>sr; i-- )
            { if( R[i].z-L[zl].z>=0 )zr=i; else break; } else
          if( W.z<-S12 )for( i=zl-1; i>sl; i-- )
...
#if 1
        { if( dir( R[sr+1]-L[sl] ).x>dir( R[sr]-L[sl+1] ).x )Ins( n )=rf[++sr];
                                                       else  Ins( n )=lf[++sl];
#else
        { if( (y=norm(R[sr+1]-L[sl]))>(x=norm(R[sr]-L[sl+1])) )
          {                            Ins( n )=lf[++sl];
      ...
      if( wl==L.len-1 && wr==R.len-1 )continue; // перехол к следующей шпации
      wl=L.len-1,wr=R.len-1; goto Reto_Deck; // к формированию надводного борта
    }
...
/* C06=0.9945218954,C08=0.9902680687,C12=0.9781476007,C18=0.9510565163,
   ...
   C48=0.6691306064,C75=0.2588190451,C78=0.2079116908,C82=0.139173101,
   C84=0.1045284633,C86=0.06975647374 ... cos(78°)=sin(12°)
   S06=C84,S12=C78,S08=C82,S04=C86 ... */
