...
//      <Пробел> раскраска поверхностей или исходная триангуляционная сетка
//
...
const Real eps=1e-6L,// ≡0.01 мм - допустимое сближение точек - к малым моделям
     ...
     hW=8.0*_Pi/3.0, // 1.134 - превышение гребня над средним уровнем моря при
      ...
      TimeStep=0.5L, // Шаг фиксирован для всех волновых структур одинаков[сек]
  tKrat=M_SQRT2*2.0L,// Дробление шага во времени в отношении исходного Куранта
...
extern Real Trun,    // Отсчет времени завершённого этапа моделирования   [сек]
            Tlaps;   // Указание отсчета времени продолжения эксперимента [час]
extern unsigned KtE, // Счётчик исполненных шагов вычислительного эксперимента
           Educt; // = управление выводами результатов мореходного эксперимента
...
extern const char *VView[];       // варианты представления потоков по обшивке
...
//
  ...
  byte Statum;   //!! Маска настройки режимов вычислительного эксперимента
                 //  0 - кинематическое позиционирование со скольжением корпуса
  ...
  struct         //!! прорисовка корпуса, вихреисточники и отражаемые импульсы
  { byte hull:2; // 0 прозрачный корпус с проявлением гидростатических центров
    ...
    byte grid:2; // ~ те же режимы Hull, но только с триангуляционными ребрами
    byte flow:2; // ~ 1 - касательные; 2 + нормальные; 3 - отражённые(+нормали)
    byte kart:1; // ~ установка одного поля графиков c картушкой сверху-справа
  } Pic;
  bool lFlow;   // ключ захвата и вовлечения корабля во внешний волновой поток
...
private:
  ...
  void Three          //       все элементы одного треугольника обшивки корпуса
     ( int Level,_Vector,_Vector,_Vector );   // в локальном корабельном базисе
  //
...
struct Waves:    // пакет групповых структур заданного периода и направления
       Matrix    // с тензорным опорным базисом направления без местоположения
...
//                                                           friend class Hull;
struct Field: //!! Class Штормовая акватория из трохоидальных волновых структур
...
//Real Tlaps;      //   Указание отсчета времени продолжения эксперимента [час]
  Real Long,Wide,  //   Протяженность и ширина модельного бассейна       [метр]
...
private:
 virtual bool KeyBoard(fixed);// справка, управление визуализацией эксперимента
...
//virtual ~Field(){};        // безусловное(бесконтрольное)завершение программы
  Field& Original( bool );   // начальная(true) и промежуточная=0 предустановка
                             // всего вычислительного эксперимента
   Real Value(Vector);  // простая билинейная интерполяция всего волнового поля
 Vector Locas(Vector); // дополнение морских координат уровнем поверхности воды
