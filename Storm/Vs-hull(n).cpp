...
{ for( int k=F.len-1; k>=0; k-- )                           // предосторожность
  { Vector &A=F[k]; A-=Ofs;                                 // привод по осадке
    ...
    if( k<F.len-1 ){ Vector B=F[k+1];                       // В - уже изменен
      if( abs( A-B )<eps && A.z<0.0 )F.Delete( k ); else    // без совпадений и
      if( A.z*B.z<0.0 )                                     // по любой стороне
      { if( Stm )e6( B=Inter( 0.0,A.z,B.z,A,B ) ); else     // вектор у штевней
...
Ok:
  ...
  if( *Str++==30 ){                  //  '\30=▲\x1E'
   ...
   Real x,y,z; int i,j,k,l,m,n;      // ... точки по циклам двойной вложенности
    ...
    //       == становятся крайними в отсчётах для шпангоутов
    //
...
//     if( R[i].x<x )R[i].y=0.0,R[i].x=x; else if( R[i].z>0.0 )R[i].y/=2.0;
      ...
      */
      ...
      //
     int wl,wr,zl,zr,
         sl=0,sr=0;   // индексы поиска по контурам шпангоутов от третьих точек
      ...
      { if( norm( L[1]-L[0] )>=norm( R[1]-R[0] ) )       // плоскости (временно)
        { Ins( n )=rf[1]; Ins( n )=lf[1];                 // обновление выборки
           for( i=2; i<L.len; i++ )
      ...
      //
      wl=sl,wr=sr;           // обратные индексные отсчеты по уровню ватерлинии
...
#if 1
//    if( sl<L.len-1 )
      for( wl=sl+1; wl<L.len && L[wl].z; wl++ );        // строго до ватерлинии
//    if( sr<R.len-1 )
//    if( L[sl]*R[sr]>0 )
      for( wr=sr+1; wr<R.len && R[wr].z; wr++ );
      if( wl==L.len )wr=R.len;
      if( wr==R.len )wl=L.len;
      zl=sl;
      zr=sr;
#else
      for( wl=sl+1; L[wl-1]*L[wl]>0 && wl<L.len; wl++ );
      for( wr=sr+1; R[wr-1]*R[wr]>0 && wr<R.len; wr++ );
      if( wl==L.len )wr=R.len;
      if( wr==R.len )wl=L.len;
      zl=sl;
      zr=sr;
...
//Break( "~n=%d[%dx%d], L=%d->%d, R=%d->%d",n,L.len,R.len,sl,wl,sr,wr );
      ...
      { //
        ...
        { if( Span( sl,sr ) )Ins( n )=rf[++sr]; // на выходе строго sl==wl и sr==wr
          ...
          if( wl-sl>2 && (dir( L[sl]-L[sl-1] )%dir( L[sl+1]-L[sl] ))<=0 //>2 от киля
           || wr-sr>2 && (dir( R[sr]-R[sr-1] )%dir( R[sr+1]-R[sr] ))<=0 )break;
        }
...
//zl=wl,zr=wr;
    ...
    Distance=-x;
//    eye=(Vector){ 45,-15 },look=(Vector){ -1,-1 };                                 //
