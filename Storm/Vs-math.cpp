 ...
 byte DrawMode;     --   0 - ватерлиния строится при любом раскладе
  ...
  Vector Ofs,      // внутренние смещения отсчетов по миделю и ватерлинии {x,z}
      ...
      Floatable, vF, // центр площади исходной и действующей ватерлинии      cF
                 vD, // условное средоточение гидродинамических пар сил<=момент
...
Real Ts=1.0;          // расчётный интервал на текущем этапе моделирования Math
...
static Real iVz=eps;  //        и величина водоизмещения при полном погружении
void
Hull::ThreeInitial()  // начальная очистка всех величин динамических параметров
{ vD=vB;              // динамические центры из предыдущего расчётного такта
  if( iF>Floatage*eps && iV>Volume*eps )vD=( vF*iF + vB*iV )/( iF+iV );
  ...
  vB=vF=Zero;         // сумматоры для задействования ранее вычисленных величин
  ...
  { Volume=Surface=Floatage=iVz=0; Gravity=vD=vC=Zero; inWater=0.0; inMass=0.0;
  }
//  else
//  { Vship=((Route[-1]-Route[-2])/(Storm->Tstep/Storm->tKrat))/Tensor(*this);// КБ
//    Wship=Whirl[-1];    // скорости выше реальные и по расчёту вращательные  в КБ
  ...
  { _Point A=Point( a ),B=Point( b ),C=Point( c );
    ...
    { iS += W;        // + скаляр площади смоченной поверхности судовой обшивки
      if( Statum>1 ){ // если внешнее давление на треугольники и пары сил  в КБ
       ...
       Real Hz=R.Z;                 // подводная вертикаль центра давления в МБ
        ...
        //
        if( Storm->Kt )
        if( Statum>2 ){    // ходовый - встречный набегающий поток воды
         Vector v = Vship  // *(1+Breadth/Length/2) - для шара получится 1,5 КБ
               + Wship*( L-vD );      // скорость треугольника, правило знаков?
          if( Statum>3 && Storm->Exp.wave ) // подводные волновые воздействия
          { Vector Q;   // выборка скорости течения под волной по координатам R
            Storm->WaveV( Storm->Trun,*(Vector*)(&R),Q );// смещение и скорость
            v -= Q/Tensor(*this);                      // частицы жидкости в КБ
          }
         Vector s=S/W;      // ориентация нормали треугольника в корабельной СК
         Real  nv=s%v;      // нормальная компонента для отражаемого течения
          if( nv<0 )        // на воде отрицательное давление реализуется плохо
          if( Kv<1.0 )nv*=( 1+(1-Kv)*(s%dir(v)) ); // сброс парадокса Даламбера
          Hz=.5*( norm( s*v ) // касательная компонента набегающего потока воды
                - norm( v )   // интегральная скорость потока для всего корпуса
            - copysign( nv*nv,nv ) )/_g;  // и составляющая к непротеканию в КБ
          vS += Hz*S;   // накопительный вектор сил давления треугольников в КБ
          vR += Hz*( ( L-vD )*S );               //++ моменты от центра тяжести
        }
      }
    ...
    { Real        // -- первые две вершины по одному борту, третья - по другому
      ...
      { Vector F; // элемент силы в глобальной морской системе координат - в МБ
        ...
        vR+=( L-vD )*F;   // разворачивающий момент дрейфа на склоне волн [Н·м]
      }
#if 0
...
{ //
  ...
  { vB /= iV;    // корабельные координаты центра величины (=подводного объема)
    ...
    vW = dir(vW);// интегральная нормаль к площади действующей ватерлинии, в КБ
    //
...
{ Vector V=Rate[-1],   // вектор полной скорости хода, качки и дрейфа в КБ
      ...
      Dir=Swing[-1], // отсчёты действующих углов x-крен y-дифферент z-рыскание
      R,U,dV=Zero;   // действующий градиент наклона волновой поверхности
  Field &Sea=*Storm;

//if( !Sea.Kt )return *this;
  //
  ...
  { Vector B=vS; // суммарное действие всех сил давления у смоченной обшивки КБ
    ...
    W.x *= (1-tanh(U.x)/2)*nM.x * (1.0-sqr(Dir.x*0.5625/_Pi));  // учетверённое
  ...
  //
  V = Tensor(*this)*V;            // переход в общую морскую систему координат
  V.rotZ( -Dir.z );               // в глобальных отсчета к носу, как к северу
  V.x -= cSp;                     // отклонение скорости идёт на демпфирование
  U = V; U &= V; U &= muF*Ts;     // кубический компонент нормируется th=-1:1
  V &= nF;                        // линейная часть демпфирования малых сдвигов
  V &= (Vector){ 1.0,1-tanh( U.y )/2,1-tanh( U.z )/2 };
  V.x += cSp-dV.x;               // покомпонентное гашение изменённых скоростей
  Speed = V.x;                   // скорость хода в МБ для информационных нужд
  V.rotZ( Dir.z );               // обновление курса с коррекцией демпфирования
  ...
  // = по сути а-ля преодоление сопротивления пропорционально квадрату скорости
  {
   ...
   Real dR=angle( Course,atan2( -Tr.y,Tr.x ) ),   // все углы от -180° до +180°
    ...
    { Kurs=false;R.z-=(dR=minmax(-dA,dR,dA));// ограничение скорости циркуляции
    }
    if( Statum )
...
//( U=Buoyancy ).z=0.0; // выбор нового местоположения вдоль маршрута движения
  ...
  if( iV>Volume*eps )                    // здесь часть корпуса касается воды..
  { dV = ( vF*iF + vB*iV )/( iF+iV );    // без ватерлинии будет центр величины
    R.x= ( 1.0+tanh(muF.x) )/2.0; U.x=iVz*( 1.0-R.x );
    R.y= ( 1.0+tanh(muF.y) )/2.0; U.y=iVz*( 1.0-R.y );
    R.z= ( 1.0+tanh(muF.z) )/2.0; U.z=iVz*( 1.0-R.z ); R*=iV;
    vC = ( dV&R )+( Gravity&U ); vC.x/=R.x+U.x; vC.y/=R.y+U.y; vC.z/=R.z+U.z;
  }
