...
Hull::ThreeInitial()  // начальная очистка всех величин динамических параметров
{
  ...
  { Volume=Surface=Floatage=0;
    Vship=Wship=Mwave=Gravity=vB=vBi=vC=dV=Zero; inWater=0.0; inMass=0.0; //{{0.0},{0.0},{0.0}};
  } else
...
//{ Vship=(Rate[-1]+Rate[-2]*2)/3;      // усредняемая скорость хода и вращения
  { Vship=Rate[-1];                     // скорость хода и вращения
    Locate=Route[-1];   // во избежание проблем от сильных внешних воздействий
    ...
    { Storm->WaveV( Trun,out(vB),Mwave ); // поток в локальном центре объема МБ
      Mwave=AtL( Mwave ); //=Mwave/Tensor(*this)  // величины осевого потока КБ
  } }
  ...
  vC=vM=vS=vR=Zero;      // метацентр, силы и моменты от перепадов уровней у борта
}
...
{ if( Level<=0 )                    //  борт, общекорабельная архитектура мимо
  { Vector A=out( a ),B=out( b ),C=out( c );
   ...
   Real   W =abs( S );              // площадь малого треугольника на обшивке
    ...
    { Matrix J; //={ {0},{0},{0} }; // временно из общей механики исключается
      ...
      if( Statum>1 ){ // если внешнее давление на треугольники и пары сил  в КБ
       ...
       Real Hz=R.z;                 // подводная вертикаль центра давления в МБ
        ...
        { Vector w= Zero,  //~~ правила знаков ?? всё необходимо заново сверять
                 ...
                 v= Wship*(L-Gravity) // скорость треугольника, правило знаков?   { + }
                  + Vship; // *(1+Breadth/Length/2) - для шара получится 1,5 КБ
          ...
          {             // выборка скорости течения под волной по координатам R
           ...
           Vector u = w;               // копия для потокового интеграла   [МБ]
            ...
            { u.z=0; u=AtL( u ); // потоковое увлечение по горизонту
              Vwave += V*u;     // всем элементам объёма(водоизмещения) корпуса  { += }
             Real rW=norm(L-vBi); // полагая слабое влияние особо близких точек
              if( rW>1 )Wwave += ( V/rW )*( (u-Mwave)*(L-vBi) ); // по объёму    { += }
            }
          }                 // нормальная компонента отражаемых корпусом частиц
         Real nv=s%v;       // на воде отрицательное давление реализуется плохо
          if( nv<0 )        //   if( Kv<1 )nv *= ( 1+( 1-Kv )*( s%dir( v ) ) );
          { if( Hz<0 && nv*nv/(_g*2)<-Hz )nv*=(1-Kv); else nv=0;  // сток/исток
          }
...
//  vP /= 2.0;    // ... что-то там то ли непонятно, то ли не совсем "чисто"...
  ...
  { if( !KtE )Break( "Ошибка инициализации\n\t(нет водоизмещения)" );
    Mwave=Vwave=Wwave=vS=vR=Zero;// летучий голландец в свободном от поддержки полёте
    vB=vC=vP=Gravity; // сам по себе над волнами
  }
  ...
  if( isDisplacement and isWaterPlane )
...
{ Vector
  ...
  { Vector B=vS; // суммарное действие всех сил давления у смоченной обшивки КБ
    ...
    //
    dV = B/Volume - AtL(Zenit);    // нормированное по единице ускорения в [КБ]
    if( norm( dV )>4.0 )dV=2*dir( dV );// максимум ускорения свободного падения
              dV *= _g*Ts;    // приращение скоростей поступательной качки [КБ]
    //
...
//!     B -= (Volume/Ts/Ts)*(R*( AtL(Route[-1]-2.0*Route[-2]+Route[-3] ) ) );     // { += }
    //
    // Моменты в ускорениях поворотов -> скорости вращения -> угловые положения
    //
  ...
  //!   переработка результатов динамического позиционирования
  //!     и автоматическое управление ходом и курсом корабля на штормовой волне
  //
 Real dR,dA;
 Vector Tr,dW=LtA( (Vector){cSp,0,0} ); dW.z=0; dW=AtL( dW );
  Vship -= dW;                    // отклонение скорости идёт под демпфирование
  U=Vship; U&=Vship; U&=Ts*muF;   // кубический компонент нормируется th=[-1÷1]
  Vship &= nF;                    // линейная часть демпфирования малых сдвигов
  Vship &=(Vector){ 1-0.5*tanh( U.x/12 ),1-0.5*tanh( U.y ),1-0.5*tanh( U.z ) };
  dV.x=0; // dW.x -= dV.x;        //.98 теперь реакция внешних сил и возмущений
  Vship += dW+dV;                 // восстановление временного исключения хода
  //
  ...
  // = по сути а-ля преодоление сопротивления пропорционально квадрату скорости
  {
    ...
    Tr = Zenit*(x*Zenit);          // курс <= перпендикуляр от правого траверза
...
/**/{//if( fabs(Direct.y+R.y)>_Ph*0.75 )R.y/=2; // ослабление дифферента ~67,5°
  ...
  Route+=( Locate += Ts*LtA( Vship+Vwave+vC*( Wwave+Wship ) ) );
                   ...
                   // поступательные скорости под действием архимедовых сил  КБ
  // Rate+= Vship; // всплытия, и градиентов давления под волновыми склонами КБ
  Rate+=AtL( dW=(Locate-Route[-2])/Ts ); dW.z=0.0;                 // КБ <=- МБ
  Mix+=(Vector){ Speed=dW%x,Course,Sea.Value(out(Zero)) };   // - для протокола
  //
 ...
 static Real Tz=0; // и посекундная выдача динамики хода на текстовую консоль
  if( Trun-Tz>1.0 ){ Tz=Trun; textcolor( LIGHTCYAN );
...
/*
  ...
  Speed = LtA( Vship )%x;         // скорость хода, отсчёты корабельного базиса
  Speed = copysign( abs( Route[-1]-Route[-2] )/Ts,Vship.x );
  Speed = copysign( abs( Vship ),Vship.x ); // abs( Vship )*( Vship.x<0?-1:1 );
*/
