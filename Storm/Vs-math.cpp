...
//     vBi,           //  положение центра величины из предыдущего шага времени
       ...
       Vwave,Wvawe;   //   потоковые скорости захватываемого волной корабля  МБ /*@*/
...
{ vD=vC;              // гидродинамический центр с предыдущего расчётного такта
  ...
  Vwave=Wvawe=Zero;   // осредняемые скорости потоков и разворотов в толще воды
...
{                //  корпус может погрузиться iF==0 или выскочить из воды iV==0
 ...
 bool isWaterPlane=iF>Floatage/32,   // существование действующей ватерлинии
  ...
  { if( !(Sea.Kt) )Break( "Ошибка инициализации\n\t(нет ватерлинии)" );
    ...
    vM=Gravity; //Metacenter; // метацентра нет, и пусть также на начальной остойчивости
  ...
  { vB /= iV;      // корабельные координаты центра величины(подводного объема)
    ...
    Vwave/=iV;     // в волновом потоке участвует объем элементарного тетраэдра
...
Vwave=Zero;
  Route+=( Locate+=Matrix(*this)*( ( V+vC*W )*Ts )+Vwave ); //+Matrix(*this)*Gravity;
