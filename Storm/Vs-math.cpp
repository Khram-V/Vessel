...
{ vD=vB;              // динамические центры из предыдущего расчётного такта
  ...
  { Volume=Surface=Floatage=iVz=0;
    Vship=Wship=Gravity=vD=vC=Zero; inWater=0.0; inMass=0.0;
  } else
        ...
        if( Statum>2 ){    // ходовый - встречный набегающий поток воды
         Vector v = Vship  // *(1+Breadth/Length/2) - для шара получится 1,5 КБ
          ...
          { Vector Q;   // выборка скорости течения под волной по координатам R
            Sea.WaveV( Sea.Trun,*(Vector*)(&R),Q );    // смещение и скорость
...
{ //
 ...
 Field &Sea=*Storm;
  ...
  { if( !(Sea.Kt) )
      Break( "Ошибка инициализации\n\t(нет ватерлинии на шаге %d)",Sea.Kt );
    ...
    if( iV>Volume )   // полное погружение в подводное положение
      ...
      vB=Buoyancy;    // центр величины <∨> сбрасывается в исходное положение и
  } else           // если судно в водоизмещающем состоянии на поверхности моря
...
{ Vector V=Rate[-1],  // вектор полной скорости хода, качки и дрейфа в КБ
       ...
       Dir=Head[-1], // отсчёты действующих углов x-крен y-дифферент z-рыскание
  ...
  { // простое статическое касание поверхности склонов волн половинных размеров
    ...
    W = (Dir-Head[-1])/Ts;         // скорость изменения курса и углов качки
  ...
  V.rotZ( -Dir.z );               // в глобальных отсчета по носу, как к северу
  ...
  Head += angle( Dir );  // величины осевых углов Крылова-Эйлера(временно) в МБ
