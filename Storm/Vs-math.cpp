...
/*                                                          ©2020-11-21 ‏יְרוּשָׁלַיִם
 ...
 Vector Gravity,       -- центр тяжести в локальных корабельных координатах  G
  ...
  //               vP, -- центры горизонтальных давлений по дрейфу и ускорению
        ...
        muF, muM;   -- демпфирование (>0) до исключения(=1) скорости качки
...
//
Hull& Hull::Moving()  // шаг во времени Ts устанавливается в Field::Simulation
{ Vector V=Rate[-1],  // поступательная - вектор полной скорости хода и дрейфа
  ...
  { Vector B=vS;  // суммарное действие давления по смоченно-изменчивой обшивке
    ...
    W.x *= nM.x * (1.0-sqr(Dir.x*0.5625/_Pi));  // учетверённое подторма́живание
    W.y *= nM.y;            // покомпонентное демпфирование угловых скоростей
    W.z *= nM.z;            // W &= nM;
    ...
    //
    V.x-=cSp; V&=nF;
//  V.x/=( 1+sqr( tanh( (cSp-V.x)/16 ) )/24 ); //+ fabs( cSp-V.x )*Ts/60 );
