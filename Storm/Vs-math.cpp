...
{ vD=vB;
  if( iF>eps )vD += ( vF-vB )/2.0; // * (0.5-(iV-Volume)/(iVz-Volume)/2);
  ...
  vB=0; vF=0;         // сумматоры для задействования ранее вычисленных величин
//vBz.x=Buoyancy.x;   // приведение абсциссы плеч тетраэдров/треугольников
  ...
  if( !Storm->Kt )    // если самое первое обращение - место конструктора Hull
    { Volume=Surface=Floatage=0.0; inWater=0.0; inMass=0.0; vD=0.0; iVz=0.0; }
//pV = norm( Vship );
}
...
//         s = S/W,                    // ориентация нормали треугольника
...
//        + a.y*b.z+a.y*c.z+b.y*c.z + a.z*b.y+a.z*c.y+b.z*c.y )/-2.0;
    ...
    { iS += W;        // + скаляр площади смоченной поверхности судовой обшивки
      ...
      { Vector Cz=L;  // здесь будет местоположение точки действия сил давления
        ...
        vR += Hz*( ( Cz-Gravity )*S ); //+ моменты от центра величины [Н·м/ρ]KБ
        //
    ...
    { Real        // -- первые две вершины по одному борту, третья - по другому
      ...
      { Vector F; // элемент силы в глобальной морской системе координат - в МБ
        ...
        vR+=F*(Gravity-L); // разворачивающий момент дрейфа на склоне волн[Н·м]
      }
...
//  E = Sea.Locas( out((Vector){0,Breadth/-4}))-A;
  ...
  { Vector B=vS;  // суммарное действие давления по смоченно-изменчивой обшивке
    //
    ...
    //
    //                                 гравитационная разность по водоизмещению
...
//  if( Statum==1 )B += iV * Zenit/Tensor(*this);
    ...
    V += dV = ( _g*Ts )    //  приращение скорости поступательной качки
            * ( B/Volume - Zenit/Tensor( *this) );
    //
    ...
    if( Statum==1 )              //? здесь или центр тяжести или центр величины
      B +=_g*vS*(Gravity-vF);    //? момент вращений от дрейфовых сдвигов на ВЛ
...
//  R = Tensor(*this)*R;    // и переход в глобальную морскую систему координат
  }                         // координатные оси корабельного базиса морской СК
    //
    ...
    //
    // V.x += 0.1*dv*Ts - dV.x;   // простоe ускорение декремента затухания м/с
    //                      вариант опытового бассейна с буксировочной тележкой
    //              или релаксация со скользящим средним по приращению скорости
  { static Real dd=0.0; dd=dd*0.96+0.04*dV.x; V.x+=(cSp-V.x+dV.x)*Ts/6.0-dd; }

    V.x = cSp-(cSp-V.x)/exp( DampF.x*Ts );// качка в сопоставлению с предыдущим
    V.y /= exp( DampF.y*Ts );      // если м=0 - проходит; м>0 - подтормаживаем
    V.z /= exp( DampF.z*Ts );     // демпфирование поступательной скорости в КБ
  //
...
//    Dir.z -= dR; // небольшое уточнение чисто для информации без привода в КБ
  ...
  { U=vB;                //       вращение относительно неподвижной толщи воды,
    if( 1.01*iV<Volume ){ U+=( Gravity-vB )*sqr( 1.0-iV/Volume ); } // или Ц.Т.
