...
static Vector         //! КБ - локальный корабельный базис; МБ - морской базис
       vBz,//vFz,     // сохраняемые центр величины сумматор для вычисления координат центра величины
...
//pV = norm( Vship );
/*if( !Field.Kt )     // если самое первое обращение - место конструктора Hull
    { Volume=Surface=Floatage=0.0; inWater=0.0; inMass=0.0; } */
}
...
{ //
  ...
  if( iF<Floatage/128 ) //! отрыв от поверхности воды (погружение или всплытие)
  { if( Storm->Kt )  // всегда должно быть водоизмещение при площади ватерлинии
    {// iV=Volume/2; // на частичное водоизмещение для приводнения после взлёта
      if( iV>Volume/128 )vB/=iV; else vB=0.0; // центр величины=центр координат
...
//    vW=Zenit/Tensor(*this); // направление вектора всплытия вертикально вверх
      ...
      vM=Metacenter;         // метацентр по начальной остойчивости
      mW=0.0;               // может быть нулём и центром базиса по определению
    } else Break( "Ошибка инициализации\n\t(нет ватерлинии)" );
...
//  vW /= iF;    // интегральная нормаль к площади действующей ватерлинии, в КБ
  ...
  //if( Statum>1 )                      // вместо приведения к центральной силе
    ...
    vM +=(Vector){ 0,0,mW.x.x/iV }/Tensor(*this); // сдвиг вверх вдоль условной
                         // аппликаты поперечного метацентра - центр кренования
//  vM.y = vB.z+mW.y.y/iV;        // и продольного метацентра ~~ удифферентовки
//  vM.x = copysign( vB.z+mW.x.x/iV,z.z );  // аппликата поперечного метацентра
//  vM.y = copysign( vB.z+mW.y.y/iV,z.z ); // продольного метацентра, дифферент
//  vM.z = vM.x-Gravity.z; // hX(z) мгновенное значение метацентрической высоты
  ...
  { Volume=iV;     // исходное водоизмещение, мгновенный погруженный объем V м³
    Surface=iS;    // подводная и смоченная поверхность корпуса            S м²
    Floatage=iF;   // исходная и действующая площадь ватерлинии            F м²
    Gravity=vM;    // центр тяжести в локальных корабельных координатах    G м
    Gravity.z-=hX; // аппликата центра масс встанет строго ниже метацентра hX м
...
// Locate=Route[-1], === текущее местоположение с учетом вертикального всплытия
      ...
      R,vW;          // действующий градиент наклона волновой поверхности
  Field &Sea=*Storm;
  //
  ...
  { Vector A,B,C,D,E;
    ...
    vW = dir( B*D + D*C + C*E + E*B ) // вектор в морской системе координат МБ
  ...
  {               //! +сделано скольжение по склонам с инерцией по ходу и качке
    ...
    Dir.y = atan2( vW.x,vW.z );    // расчёт угла дифферента
    Dir.x = atan2(-vW.y,vW.z );    // выбор нового угла крена
...
//  if( Statum>1 )             //?? пока исключено по причине сильного рыскания
    ...
    W.x *= 1/exp( DampM.x*( 1+2*fabs(Dir.x)/_Pi )*Ts );
    W.y *= 1/exp( DampM.y*Ts );                //! демпфирование полных величин
    W.z *= 1/exp( DampM.z*Ts );                //        угловых скоростей
...
//      R.z = copysign( min( 0.11,fabs( R.z ) ),R.z ); //  6°
    if( Statum )
    {   rotY( R.y );         // ++дифферент в MБ - всё теперь в дифференциалах
        rotX( R.x );         // ++крен      в KB = много вопросов при аппликате
    }   rotZ( R.z );         // ++рыскание  в ?? ~ отклонение курса в вертикали
    //
  ...
  vW=vB; vW.x=Gravity.x;
  Route += ( Locate+=Matrix(*this)*((V+vW*W)*Ts) ); // + Matrix(*this)*Gravity;
...
//Z = Locate.z - ( Dir * (Matrix(*this)*vW) ).z;
