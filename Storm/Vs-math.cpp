...
//{ Vship=((Route[-1]-Route[-2])/Ts)/Tensor(*this); // скорость хода и вращения
  { Vship=Rate[-1];    // скорость хода и вращения
    ...
    { Storm->WaveV( Storm->Trun,Tensor(*this)*vB,Mwave ); // величины для    МБ
      Mwave=Mwave/Tensor( *this );                        // осевого потока  КБ
    }
  }
  ...
  { _Point A=Point( a ),B=Point( b ),C=Point( c );
   ...
   Real   W =abs( S );              // площадь малого треугольника на обшивке
    ...
    Vector L =( a + b + c )/3.0,    // центр площади к треугольнику обшивки  КБ
           M = L * 0.75,            // центр объема элементарного тетраэдра  КБ
    ...
    Real   V =( c % S )/3.0;        // водоизмещение элементарного тетраэдра
           ...
           vB += M*V;               // векторная сборка статических моментов КB
...
//  vP /= 2.0;    // ... что-то там то ли непонятно, то ли не совсем "чисто"...
  ...
  { Volume=iV;     // исходное водоизмещение, мгновенный погруженный объем V м³
    ...
    DampInit();
  } else  //! Kt≠0 предотвращает повторы вычислений начальных данных по корпусу
...
{ Vector
  ...
  { // простое статическое касание поверхности склонов волн половинных размеров
    ...
    Vship.z = ( R.z-Z )/Ts;        // скорость по вертикальной качке
    Z = R.z;                       // изменение аппликаты посадки на миделе КВЛ
    Wship = (Direct-Head[-1])/Ts;  // скорость изменения курса и углов качки
  ...
  { Vector B=vS; // суммарное действие всех сил давления у смоченной обшивки КБ
    ...
    if( Route.len>3 )
    B += (Volume/Ts) * ( R*( ((Head[-1]-Head[-2])/Tensor(*this)) * Vship ) );
//  B += (Volume/Ts/Ts)*(R*((Route[-1]-2.0*Route[-2]+Route[-3])/Tensor(*this)));
    //
    //  Моменты в ускорения поворотов -> скорости вращения -> угловые положения
    //
    Wship+=Ts*(B/Tensor(inMass));//угловые дифференциалы приращения поворотов/ρ
    ...
    U=Wship; U&=Wship; U&=muM*Ts; // большие скорости ослабляются степенью куба
    ...
    R = Wship*Ts;                // здесь приращение собственно углов качки  КБ
  }
  ...
  //
  Vship = Tensor(*this)*Vship;    // переход на общую морскую систему координат
  Vship.rotZ( -Direct.z );        // в глобальных отсчета по носу, как к северу
  Vship.x -= cSp;                 // отклонение скорости идёт под демпфирование
  U=Vship; U&=Vship; U&=muF*Ts;   // кубический компонент нормируется th=[-1÷1]
  ...
  Vship = Vship/Tensor(*this);   // возврат из МБ в КБ
  //
  ...
  // = по сути а-ля преодоление сопротивления пропорционально квадрату скорости
  {
   ...
   Vector Tr = Zenit*(x*Zenit);    // курс <= перпендикуляр от правого траверза
    ...
    if( Statum )
    {//if( fabs(Direct.y+R.y)>_Ph*0.06 )R.y/=2; // ослабление дифферента 54÷72°
...
//  if( fabs( Direct.y )>_Ph*0.5 )Direct.y=copysign( _Ph*0.5,Direct.y );
   ...
   static Real Tz=0; // и посекундная выдача динамики хода на текстовую консоль
    if( Sea.Trun-Tz>1.0 )
    { Tz=Sea.Trun; textcolor( LIGHTCYAN );
  ...
  Rate += Vship;    // поступательные скорости под действием архимедовых сил КБ
...
*/
//Route+=( Locate+=Matrix(*this)*( Vship*Ts ) );     // ~-~ без проворачивания
//Vwave=Zero;
//Wwave=Zero;
  Route+=( Locate+=Matrix(*this)*(Vship+vC*(Wship+Wwave)+Vwave)*Ts ); //+Matrix(*this)*Gravity;
