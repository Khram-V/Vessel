...
{ if( Level<=0 )                    //  борт, общекорабельная архитектура мимо
  { Vector A=out( a ),B=out( b ),C=out( c );
   ...
   Real   W =abs( S );              // площадь малого треугольника на обшивке
    ...
    { Matrix J;//=0.0;//={{0},{0},{0}};    // временно из общей механики исключается
...
//Vship &= (Vector){ 1.0-0.5*tanh( U.x ),1.0/cosh( U.y ),1.0/cosh( U.z ) };
...
#else
 Vector dA=LtA( (Vector){cSp,0,0} ); dA.z=0; dA=AtL( dA );
  Vship -= dA;                    // отклонение скорости идёт под демпфирование
  U=Vship; U&=Vship; U&=Ts*muF;   // кубический компонент нормируется th=[-1÷1]
  Vship &= nF;                    // линейная часть демпфирования малых сдвигов
  Vship &=(Vector){ 1/*-.5*tanh( U.x )*/,1-.5*tanh( U.y ),1-.5*tanh( U.z ) };
   dA.x -= dV.x;                  //.98 теперь реакция внешних сил и возмущений
  Vship += dA+dV;                 // восстановление временного исключения хода
...
/**/{//if( fabs(Direct.y+R.y)>_Ph*0.75 )R.y/=2; // ослабление дифферента ~67,5°
  ...
  Route+=( Locate += Ts*LtA( (R=Vship+Vwave)+vC*( Wwave+Wship ) ) );
...
//Rate+=R-Vwave/2; // поступательные скорости под действием архимедовых сил  КБ
