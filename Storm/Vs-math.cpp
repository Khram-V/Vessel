...
{ //vD=vB;              // динамические центры из предыдущего расчётного такта
  ...
  { Volume=Surface=Floatage=iVz=0;
    Vship=Wship=Gravity=vC=Zero; inWater=0.0; inMass=0.0; //vD=Zero;
  } else
   ...
   Real   W =abs( S );                // площадь малого треугольника на обшивке
       ...
       Real Hz=R.Z;                 // подводная вертикаль центра давления в МБ
        ...
        if( Statum>2 ){    // ходовый - встречный набегающий поток воды
         Vector v = Vship  // *(1+Breadth/Length/2) - для шара получится 1,5 КБ
                  + Wship*L; // ( L-vD ) скорость треугольника, правило знаков?
         ...
         Real  nv=s%v;      // нормальная компонента для отражаемого течения
          ...
          vR += Hz*( L*S );   // ( L-vD )*S );  // ++ моменты от центра тяжести
        }
      }
    ...
    { Real        // -- первые две вершины по одному борту, третья - по другому
      ...
      { Vector F; // элемент силы в глобальной морской системе координат - в МБ
        ...
        vR+=L*F;  //(L-vD)*F разворачивающий момент дрейфа на склоне волн [Н·м]
      }
#if 0
