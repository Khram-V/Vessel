...
//{ Vship=((Route[-1]-Route[-2])/Ts)/Tensor(*this); // скорость хода и вращения
  { Vship=Rate[-1];    // скорость хода и вращения
    ...
    { Storm->WaveV( Storm->Trun,LtA( vB ),Mwave ); // величины для    МБ
       ...
       Real Hz=R.Z;                 // подводная вертикаль центра давления в МБ
        ...
        { Vector C=Zenit*( Sz/W );  // длина к синусу угла вертикали с нормалью
          ...
          { Real az=a.Z-a.w, bz=b.Z-b.w, cz=c.Z-c.w,  // погружение вершин в МБ
            ...
            C=(d/abs( C ))*( dir( C*Sz ) );  // к центру действия силы давления
        ...
        if( Statum>2 ){   // ходовый+волновой - встречный набегающий поток воды
         Vector w= Zero,  ///~~ правила знаков ?? всё необходимо заново сверять
          ...
          {             // выборка скорости течения под волной по координатам R
            Sea.WaveV( Sea.Trun,*(Vector*)(&R),w );  // скорость потока в волне
           Vector u = w;               // копия для потокового интеграла   [МБ]
            ...
            { u.z=0; u = AtL( u ); // потоковое увлечение по горизонту
              Vwave += V*u;  // по всем элементам объёма(водоизмещения) корпуса
...
//  vP /= 2.0;    // ... что-то там то ли непонятно, то ли не совсем "чисто"...
  ...
  { Vector A=LtA( vB ),                 // Во внешней морской системе координат
           B=LtA( Gravity );            // действующих центров по полной массе
...
//  if( norm( dV )>4.0 )dV=dir( dV );  // максимум ускорения свободного падения
    ...
    if( Route.len>3 )
    B += (Volume/Ts) * ( R*( AtL( Head[-1]-Head[-2] ) * Vship ) );
...
//Route+=( Locate += Matrix(*this)*(Vship+vC*(Wship+Wwave)+Vwave)*Ts ); //+Matrix(*this)*Gravity;
  Route+=( Locate += Ts*LtA( Vship+vC*(Wship+Wwave)+Vwave) ); //+Matrix(*this)*Gravity;
