...
Ok: Str=stringData( Fh );            // Длинная строка в буфере входного файла
  ...
  { Real x,y,z;                      // здесь продолжается серия проверок
    ...
    Keel[0]=Keel[1]; //-eps-2;
//    if( !Stern.length )
//    { for( i=0; i<Frame[1].length; i++ ){ (Stern+=Frame[1][i]).x=Keel[0]; Stern[i].y=0; }
////    Frame[1].length=0;
//    }
//    else
...
//  if( !Stem.length )
//    for( i=0; i<Frame[Nframes].length; i++ )
//         (Stem+=Frame[Nframes][i]).x=Keel[Nframes+1];
//  else
    for( i=0;; )
    if( i>=Stem.length )break; else
    { Vector &S=Stem[i];
      if( S.x>Keel[Nframes+1] )Keel[Nframes+1]=S.x; // экстремум на форштевне
      if( S.x>Keel[Nframes] )
      {
//        Frame[Nframes+1] += Stem.Delete( i ); i--;  // образование шпангоута
        Frame[Nframes+1]+=S;   // образование шпангоута
//      Stem.Delete( i );
      } //else
      i++;
    }
