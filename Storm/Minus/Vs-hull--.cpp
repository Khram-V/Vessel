...
//{ if( k<Frame[1].length )               // на последовательное перечисление
//    { R=Frame[1][m=k++]; return true; } // пробная точка правее ахтерштевня
//} else
...
//if( s==Nframes+2 )           // правее шпангоутов нет, есть только форштевень
//{ if( k<Frame[s].length )    // обычная поисковая фиксация ++ по перечислению
//    { L=Frame[s][k]; m=LeftFrame|k++; return true; }
//} else
//if( s>0 )                          // вариант со средними шпациями-трапециями
  { _Real Fx=Keel[s]; Flex &G=Frame[s-1];
    for( int j=-1,i=0; i<Stern.length; i++ )if( !St[i] )
    { _Vector S=Stern[i];   // break;
...
Ok: Str=stringData( Fh );            // Длинная строка в буфере входного файла
  ...
  { Real x,y,z;                      // здесь продолжается серия проверок
    ...
    for( i=0; i<=Nframes+1; i++ )Frame[i].length=0; // вычистка всех шпангоутов
    ...
    //
    //    Экстремумы для килевой линии и крайние шпангоуты
    //
    Keel[0]=Keel[1];
    for( i=0; i<Stern.length; i++ ){ Vector &S=Stern[i];
...
//    if( !Stem.length )
//    for( i=0; i<Frame[Nframes].length; i++ ){ Vector &F=Frame[Nframes][i];
//      if( F.y )(Frame[Nframes+1]+=F).y=1; //Frame[Nframes+1][i].x+=1;
//    }
//    else
    for( i=0; i<Stem.length; i++ ){ Vector &S=Stem[i];
...
*/
 ...
 // Shell=(unsigned**)Allocate( Nframes+3,24*sizeof(unsigned) );  //! двумерный
   ...
   byte *St=(byte*)calloc( sizeof( byte ),Stern.length+2 ), // признаки учёта
   ...
   Real LSP=Length/Nframes;// условное усреднённое пространство (=длина) шпации
    for( n=1,mzR=-1; n<=Nframes+1; n++ )
    { mzL=mzR; k=0;         // фальшборт поднимается выше низкой главной палубы
      if( n<=Nframes )      //    правый шпангоут предустанавливается очищенным
       for( i=1,mzR=Frame[n][0].z; i<Frame[n].length; i++ )        // штевни не
        if( !k ){ if( mzR>Frame[n][i].z )k=1; mzR=Frame[n][i].z; } //   трогаем
             else if( mzR>Frame[n][i].z )mzR=Frame[n][i].z; if( !k )mzR=-1.0;
      if( n>1 )  // оставляем первый бит для точек без повторов на пересечениях
...
#define L_plus {Ins(n)=ml;ls=LS,L=Ll;sl=LNext(n,ml,Ll,St);Ll.x=0.0;if(!(LS=Ll-L))NY=NR;else NY=NL;}
#define R_plus {Ins(n)=mr;rs=RS,R=Rr;sr=RNext(n,mr,Rr,Sm);Rr.x=LSP;if(!(RS=Rr-R))NY=NL;else NY=NR;}
...
//        if( OPT )L_plus else R_plus
   ...
   Vector L,R,   //WS,         // базовые точки, верхняя грань четырехугольника
    ...
    for( n=1,mzR=-1; n<=Nframes+1; n++ )    // шпангоуты в поисковых алгоритмах
    {
    ...
    //
      ...
      if(n>1) // оставляем только первый бит, то без повторений на пересечениях
...
#define Lskip(W){ W=Ll; while(kl<wl.length)if( W!=(Ll=InShell(n,ml=wl[kl++])) )break; }
#define Rskip(W){ W=Rr; while(kr<wr.length)if( W!=(Rr=InShell(n,mr=wr[kr++])) )break; }
#define Lstep(W){ if(kl<=wl.length){ Ins(n)=ml; if(kl<wl.length)Lskip(W) } }
#define Rstep(W){ if(kr<=wr.length){ Ins(n)=mr; if(kr<wr.length)Rskip(W) } }
      //
      //  первые одна/две точки в основание шпации устанавливаются обязательно
      //     ... шпангоут не может состоять из одной точки
      //
      if( wl.length>0 ){ Ll=InShell( n,ml=wl[kl++] ); Lstep( L ) }  // левой
      if( wr.length>0 ){ Rr=InShell( n,mr=wr[kr++] ); Rstep( R ) }  // правой
...
//      }
        ...
        { if( B==(C=InShell( n,wl[nl++] )) )           // поиск сломов борта
          { if( nl==kl+1 )kl++;                          // и совпадающих точек
            while( nl<wl.length )
            if( B!=(C=InShell(n,wl[nl++])) )break; else if( nl==kl+1 )kl++; break;
        ...
        { if( B==(C=InShell( n,wr[nr++] )) )
          { if( nr==kr+1 )kr++;
            while( nr<wr.length )
            if( B!=(C=InShell(n,wr[nr++])) )break; else if( nr==kr+1 )kr++; break;
...
#define mnx( w,n ) w[ minmax( 0,n-1,w.length-1 ) ]           /// (n-1) ??? => n
        ...
        ( A=dir((B=InShell(n,mnx(wr,nr)))-InShell(n,mnx(wl,nl))) ).x=1; //A.z*=0.86;
        if( fabs( A.z )<0.036 )continue;  /// ??~~ слабый алгоритм - переделать
        for( i=(k=kl)+1; i<nl; i++ )
        { ( C=dir( B-InShell( n,wl[i] ) ) ).x=1;                          //C.z*=0.86;
          if( norm( C )<=norm( A ) ){ A=C; k=i; }
        } if( k>kl )nl=k; if( nl==kl && nl<wl.length )nl++;

        ( A=dir(InShell(n,mnx(wr,nr))-(B=InShell(n,mnx(wl,nl)))) ).x=1; //A.z*=0.86;
        for( i=(k=kr)+1; i<nr; i++ )
        { ( C=dir( InShell( n,wr[i] )-B ) ).x=1;                          //C.z*=0.86;
          if( norm( C )<=norm( A ) ){ A=C; k=i; }
        } if( k>kr )nr=k; if( nr==kr && nr<wr.length )nr++;
      }
      if( kr<=wr.length )Ins( n )=mr;   //!?? выход из цикла не очень чист...
      if( kl<=wl.length )Ins( n )=ml;
    }
#endif
...
//printf( "\n Это выход %d, %d, %d, %d...",Frame[0].length,Frame[1].length,Frame[2].length,Frame[3].length ); getch();
