...
{ Vessel=this;                  // ссылка на корпус, пусть будет внешний доступ
  ...
  DampM=(Vector){ 0.2,0.3,0.4 };  /// ... по бортовой, килевой качке и рысканию
...
{ Hull &Ship=*Vessel;                    // Field &Sea=*Storm;
  ...
  { case _BkSp:                          // временное прекращение\возобновление
       { static byte wave=0;             //   морского волнения (если оно было)
         Field &S=*Storm;
         wave|=S.Exp.wave<<2; S.Exp.wave=wave&3; wave>>=2; //
         if( logTime() )
         { if( wave )fprintf( VIL," ⇒ исключение морского волнения [%d]",wave ); else
           if( S.Exp.wave )fprintf( VIL," ⇒ возобновление: «%s»",ModelWave[S.Exp.wave]);
