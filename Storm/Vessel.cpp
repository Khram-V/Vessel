...
{ Vessel=this;                  // ссылка на корпус, пусть будет внешний доступ
  Nframes=Mid=0; Length=Breadth=Draught=1.0; Trim=0.0;
  DampF=(Vector){ 0.04,0.3,0.4 }; /// демпфер по абсциссе, ординате и аппликате
  DampM=(Vector){ 0.1,0.3,0.4 };  /// ... по бортовой, килевой качке и рысканию
  mx=my=0;                        // и мышку тоже в исходное
...
{ const
 ...
 Hull &V=*Vessel; // копия ссылки доступа к определению цифровой модели корабля
  ...
  Exp.draw=0; // 0 - 3 метод закраски или сеточной прорисовки морского волнения
...
*/Trun=Tlaps=0.0;            // отсчет реального времени моделирования[сек,час]
  ...
  //
  for( int l,i=0; i<2; i++ ) // повторение выборки для Vessel.vil и <Model>.vil
  { strcpy( fname( strcpy( Lst,V.FileName ) ),"Vessel.vil" );
    ...
    if( VIL )                // к настройке модели
    while( !feof( VIL ) )
    { char *s=getString( VIL ); if( s[0]==';' || !memcmp( s,"//",2 ) )continue;
      while( *s && *s<=' ' )s++; if( !strcut( s ) )break; // чисто к завершению
      //
...
{ Hull &Ship=*Vessel;                    // Field &Sea=*Storm;
  ...
  { case _BkSp:                          // временное прекращение\возобновление
       { static byte wave=0;             //   морского волнения (если оно было)
          wave|=Storm->Exp.wave<<2; Storm->Exp.wave=wave&3; wave>>=2;// возврат
          Storm->Original( false ); } break;   // вычислений в начальный момент
...
{ const static char                                  // настройкам и методам
  ...
  { case _F1: Help( Id,Cmds,Plus,1,1 ); break;            // справка с корпусом
    ...
    case _F8: Statum=Model_Config( Statum,this ); break;
...
{ const static char                                     // Краткая подсказка по
  ...
  { case _F1: Help( Id,Cmds,Plus,-1,-1 ); break;
    ...
    case _F8: Vessel->Statum=Model_Config( Vessel->Statum,this ); break;
...
{//            _clearfp();
 ...
 //    setlocale( LC_CTYPE,".UTF8" );
  textsize( 92,36 ),
  texttitle( "Штормовая мореходность корабля (вычислительный эксперимент)" ),
  textcolor( YELLOW ),printf( " Штормовая мореходность корабля " ),
  textcolor( WHITE  ),printf( " (вычислительный эксперимент)" ),
  ...
  textcolor( CYAN ),printf(
    "\n «МИДВ» ©1975-2022 Калининград – Севастополь – Сахалин – יְרוּשָׁלַיִם"
    "  В.Храмушин" ),textcolor( LIGHTGREEN ),cprint( 1,27,
...
{ for( int y=10; y<=12; y++ )   //  при изменении параметров волн Field::Config
  { Waves &W = y==10 ? Storm->Wind : ( y==11 ? Storm->Swell : Storm->Surge );
    ...
    :"  %3.0f м,   %4.1f\",   %4.2f м,   %4.1f м/с,     %3.0f°,    %.1f м [%d·%d] \n",
     ...
     W.Ds,W.Mx,W.My,Storm->Long,Storm->Wide );
} }
//inline void printB(unsigned B){ if(B>>1)printB(B>>1); putch((B&1)?'1':'0'); }

void Hull::wPrint()   //! информация по смоченному корпусу на текстовой консоли
{ Field &F=*Storm;    //++ текстовые данные о состоянии и динамике корабля
  textcolor( YELLOW ),
  cprint( 1,14," Time %s + %3.2f\"      \n"
               ...
               " Surface %1.0f << %1.0f  \n"
               " Floatable %1.0f << %1.0f  \n"
               " μCenter %3.1f >> %3.1f >> %3.1f Gravity  \n"
               "       h %3.1f >> %3.1f    H %3.1f >> %3.1f ",
          DtoA(F.Kt*F.Tstep/F.tKrat/3600,3),F.Tstep,Speed*3600/_Mile,
          Speed/sqrt(_g*Length),sqr(Speed)*_Pd/_g/Length,Volume,iV,
          Surface,iS,Floatage,iF,Metacenter.x,vM.x,Gravity.z,
          hX,vM.z,Metacenter.y-Gravity.z,vM.y-Gravity.z ),
  cprint( 79,8,"Status{ %X } ",Statum ), // printB( Statum );
