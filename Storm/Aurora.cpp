...
//Ts=Tstep/tKrat;            // оценочный интервал временного шага эксперимента
  ...
  { int M,D,Y; Instant.UnPack( M,D,Y );
    ...
    if( Educt&0xFF )               // если есть что писать в протокол испытаний
    { fprintf( VIL," эксперимент, в записях %s:",
               Educt&0x200 ? "регистрация только экстремальных событий" :
                             "учёт всех максимумов измеряемых величин" );
      fprintf( VIL,"\n %s, %s; %s, %s и %s качка; ускорения %s, %s и %s\n",
               Educt& 1? "ходкость"    : "—ход",
               Educt& 2? "рыскание"    : "—рыск",
               Educt& 4? "вертикальная": "—верт",
               Educt& 8? "бортовая"    : "—борт",
               Educt& 16?"килевая"     : "—кил",
               Educt& 32?"в корме"     : "—корм",
               Educt& 64?"на миделе"   : "—мид",
...
{ Diagonal = Trun*Cw + Phase*Length;          // дистанция свободной границы [м]
...
{ for( int y=0; y<mY; y++ )      // предварительная разметка сеточных координат
  ...
  if( init || Exp.wave>=2 )
    { if( logTime() )fprintf( VIL," ⇒ возобновление ⇒ 0\"[0]\n" );
      Trun=Tlaps=0.0; KtE=0;     // реальное время для моделирования [сек, час]
...
{ static int recurse=0;                  // по специальному таймеру
  if( Active_Key )Active_Key&=Storm->Ready() && Vessel->Ready();// ++WinReady()
...
{ if( Active_Key &= Storm->Ready() && Vessel->Ready() ) // WinReady()
