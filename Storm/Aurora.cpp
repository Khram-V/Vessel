...
static bool Active_Key=true; // ключик позволяет выйти из процесса без закрытия
//!
...
{ static int recurse=0;                  // по специальному таймеру
  Active_Key=Active_Key && Storm->Ready() && Vessel->Ready();   // ++WinReady()
//Active_Key=Active_Key && WinReady(Storm) && WinReady(Vessel); // ++WinReady()
...
{ static unsigned oKt=0,i=0;
   ...
   Active_Key=Active_Key && Storm->Ready() && Vessel->Ready();   // ++WinReady()
   if( Active_Key )if( oKt==Storm->Kt )   // принудительный перезапуск таймеров
   { Storm->SetTimer( 100 );
