...
//   ╞═══════════════════════════════════════════════╝
//   └── ©2018-25, Санкт-Петербург — Южно-Сахалинск — Могилев — Друть — ‏יְרוּשָׁלַיִם
//
static const char
...
CopyRight[]="\n «МИДВ» ©1975-2024 Калининград–Севастополь–Ленинград–Сахалин"
            " – יְרוּשָׁלַיִם – В.Храмушин",
Title_Hull[]=" Аксонометрический вид корпуса корабля и профилей морских волн ",
Title_Wave[]=" Корабль и трохоидальные штормовые структуры морских волн ";
const char
...
byte Mekhanik_Status=4,  //  режим гидро\механики хода и маневрирования корабля
...
Hull::Hull():Matrix(),View// в прицепе View окошко графической визуализации OpenGL
...
{ Vessel=this;                  // ссылка на корпус, пусть будет внешний доступ
  ...
  color(yellow); AlfaVector( 9,0.1 ).Print( -1,0,
...
//Ts=Tstep/tKrat;            // оценочный интервал временного шага эксперимента
  ...
  View_initial( hypot( Long,Wide )/2.0 ); // пара люстр света не полудиагоналях
  ...
  { int M,D,Y; Instant.UnPack( M,D,Y );
    ...
    { fprintf( VIL," эксперимент, в записях %s:",
               V.Educt&0x200 ? "регистрация только экстремальных событий" :
                               "учёт всех максимумов измеряемых величин" );
...
//glLightModelfv( GL_LIGHT_MODEL_AMBIENT,(const float[]){.2,.2,.2,1 } );
  eyeX=150,eyeY=-16.4,eyeZ=0; lookX=0,lookY=-10,lookZ=0;  // под обзор
...
{ Identity(); axiY( Trim ); rotZ( -Course ); return *this;
}
Hull& Hull::Original()
{ if( !Storm )return *this;          // здесь пролог может быть ещё не завершён
  ...
  Route.len=Rate.len=Head.len=Whirl.len=Mix.len=0;   // начальная инициализация
...
{ Hull &Ship=*Vessel;                    // Field &Sea=*Storm;
  ...
  { case _BkSp:                          // временное прекращение\возобновление
    ...
    case '1': case '!':{ Vector W={_Ph/3.0}; W=Ship.LtA( W ); // вызов Крылова в МБ
                         Ship*=Krylov( W.x,W.y,W.z ); } break; // задаётся
    case '3': case '#':{ Vector W={-_Ph/3.0}; W=Ship.LtA( W ); // импульс крена ±30°
    ...
    case '5': case '%': angle(Ship.Course=-Ship.Head[-1].z);  // так держать
                                                       break;
    default: return false;   // _Esc и все иные запросы клавиатуры сбрасываются
  } return true;
}
...
{ const static char                                 // настройкам и методам
     ...
     *Cmds[]={ "F1 ","- краткая справка",
               ...
               "F4 "," настройка гидромеханики хода",
               "F5 "," изображение обшивки и потока",
  ...
  { case _F1: Help( Id,Cmds,Plus,1,1 ); break;            // справка с корпусом
    ...
    case _Esc: Above();                            // установка активности окна
     if( MessageBoxW( 0,L"Завершение вычислений",L" Aurora :: Ship", // ~~ hWnd
...
{ const static char                                 // Краткая подсказка по
  ...
  *Cmds[]={ "F1 -"," краткая справка",
  ...
  { case _F1: Help( Id,Cmds,Plus,-1,-1 ); break;  // краткая подсказка
    case _F4: Config(); break;                   // меню параметров и режимы
    ...
    case _Esc: Above();                // установка активности окна
     if( MessageBoxW( 0,L"Завершение эксперимента",L" Storm :: Sea",
         MB_APPLMODAL|MB_ICONQUESTION|MB_OKCANCEL|MB_DEFBUTTON1 )==IDOK )
         Close(); return true;
...
{ static int recurse=0;                  // по специальному таймеру
  if( WinReady() )    // предварительное завершение всех ранее начатых процедур
...
//Start_Experiment();    /// !!! - считывание конфигурационного файла !!!
  ...
  if( Ship.Gravity.z>Ship.Draught )                               // и др ...
     Ship.hX=Ship.Metacenter.z=Ship.Metacenter.x-(Ship.Gravity.z=Ship.Draught);
     Sea.Kt=0;            // исходная гидростатика затем будет перепроверяться
...
//  Sleep( 1000 );      // или варианты полной задержки по блокирующему таймеру
