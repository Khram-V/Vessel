...
//
#include "Vessel.h"                 // графическая среда OpenGL - Window-Place
...
//
  ...
  color( blue ); Print(
   Exp.wave==1 ? " моделирование гидродинамики на теоретическом волнении" :
   Exp.wave==2 ? " вычислительный эксперимент с отражением волн от корпуса" :
   Exp.wave==3 ? " эксперимент с учётом скоростей течений и дисперсией волн"
               : " чистое штилевое море без волнения и ветра" );
 _Real S=Vessel->Speed,L=Vessel->Length; //byte &B=Vessel->hiFlow;
  color( black ); Print( 2,2,Model[Vessel->Statum] );
//  (B&1) ?"Кинематика по хордам 4-х точек скольжения над волновыми склонами" :
// !(B&10)?"Вертикальная гидростатика: бортовая-килевая-вертикальная качка" :
//           "Архимедовы градиенты на склонах волн: все шесть видов качки" );
  Print( 2,3,
