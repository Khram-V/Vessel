...
//Vector dirZ( _Real a ){ return (Vector){ sin(-a),cos(a),0 }; }
/*w=x*c-y*s; y=y*c+x*s;
...
{ arrow( (Vector){ 0,0,-Z },(Vector){ 0,0,Z },0.0l,clr ),
  ...
  P.Text(_North,0,0,Z,z).Text(_North,0,Y+L/100,0,y).Text(_North_East,L*1.01,0,0,x);
}
//
...
//if( x==x1 || x0==x1 )return y1;                       // повторное совпадение
  if( x0==x1 )return 0.5*(y0+y1);                       // среднеарифметическое
