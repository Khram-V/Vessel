...
{ if( N>=2 )                            //     если косинус угла гладкости > La
  { complex a,b; Real la,lb; int i,k; Easy();
    for( k=1,i=2; i<=N; i++ )                     // N=N-1 или последний отсчёт
    { a.x=z[k]-z[k-1]; a.y=y[k]-y[k-1]; la=abs( a );
      if( la>=fabs( Lc ) )k++; else        // превышение заданного модуля длины
      if( la>eps || Lc<0 )               // случай сохранения совпадающих узлов
      { b.x=z[i]-z[k]; b.y=y[i]-y[k]; lb=abs( b );
        if( (a%b)/(la*lb)<Ac )k++;        // косинус меньше у заострённого угла
      }
      if( k<i )z[k]=z[i],y[k]=y[i];       // сдвиг-копирование по необходимости
    } N=k;
} }
...
//
void Hull::Init()
{ const int mZ=296,mX=720;     // простой расчет полного объема корпуса
 Real x,Dx,dx=Lmx/(mX+1),      //      (по внутренним трапециям)
      z,Dz,dz=(Draught-Do)/mZ, // шаг по аппликате с учетом заглубления бульба
      y=0, ds=dx*dz,           // ордината с элементарной площадкой обшивки
      Vs=0,Vm=0;               // площадь шпангоута
      Volume=Surface=0;        // водоизмещение и смоченная поверхность корпуса
 int i,j;
...
//      if( !i || i==mX )Surface -= y/2;        // ... снова к методу трапеций
      }
    } Volume += Vs*ds*2;
