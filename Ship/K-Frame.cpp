...
//if( (B=1.0-(A/=h))<eps )Y=y[k+1],Z=z[k+1]; else // выход с найденной  точкой
  ...
  { Real h=_x[k+1]-_x[k],B=1.0-(A/=h);
    Real b=( B*B-1.0 )*B,
         a=( A*A-1.0 )*A;
    Y = B*y[k]+A*y[k+1] + ( b*_y[k]+a*_y[k+1] )*h*h/6.0;
    Z = B*z[k]+A*z[k+1] + ( b*_z[k]+a*_z[k+1] )*h*h/6.0;
  } else
  { Y = y[k] + A*( y[k+1]-y[k] );              // простая линейная интерполяция
    Z = z[k] + A*( z[k+1]-z[k] );
} }
...
{ if( N<1 )return y[0];                          // одна точка на другие случаи
  ...
  { if( k>=N )return bound?y[N]:0.0;             // теперь точка выше шпангоута
    if( az<=z[k+1] )                             //       линейная интерполяция
#if 0
      return y[k]+(az-z[k])*(y[k+1]-y[k])/(z[k+1]-z[k]);  // или чисто линейное
#else
    {//eal X=( _x[k+1] - (z[k+1]-az)*(_x[k+1]-_x[k])/(z[k+1]-z[k]) )/_x[N],Y,Z;
      Real X=( _x[k]+(az-z[k])*(_x[k+1]-_x[k])/(z[k+1]-z[k]) )/_x[N],Y,Z;
           YiZ( X,Y,Z ); return Y; }
#endif
} }
...
{ Real A,S,a=Asx.G( z,true ),s=Stx.G( z,true ); int k;  // граничного шпангоута
  ...
  if( x<F[0].X ){ A=Asy.G( z ),s=F[0].X,S=F[0].G(z); } else
  if( x>F[Ns-1].X ){ S=Sty.G( z ),a=F[Ns-1].X,A=F[Ns-1].G(z); } else
