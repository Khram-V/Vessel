...
( Real *C,            // собственно прорисовываемая кривая (NULL - если ее нет)
 ...
 Real dL,Y,uY;
  ...
  if( C )
  { glBegin( GL_LINE_STRIP ); for( int i=0; i<nZ-1; i++ )glVertex2d( C[i],i );
...
{ int i,k;
 Real dA=5*M_PI/180.0,z,dz=AxisStep( 0.5*(Lmax-Lmin) ); wT->Focus();
  ...
  for( k=1,z=-dz/2; z>=Lmin; k++,z-=dz/2 )
     glVertex2d( wT->X( (k%2)?6:12 ),z ),glVertex2d( 0,z );
  glEnd(); gl_CYAN;
  stWin->Alfabet( 18,"Times",400,true ).Text( _East,wT->X( 32 ),Lmax,0,
...
#pragma omp parallel for reduction(+: V,Mx,My )
    ...
    if( V>0 )aC[j][k]=(complex){Mx,My}/V; // Координаты новых центров величины
...
{ int i,j,k;                                  // статической остойчивости формы
  ...
  Axis_Statics( A );                                      // верхний экстремум
  for( k=w1*2; k<=wN*2; k++ )
  { if( !(k&1) )
