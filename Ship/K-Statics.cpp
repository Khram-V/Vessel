...
{ WinStability( int W,int H ):
  Window( "Кривые остойчивости с элементами теоретического чертежа",0,0,W,H ){}
...
//  if( !stWin->Ready() )return; else
    ...
    if( !k && j%(nA/16)==1 )stWin->Show();    //!!! что-то в FLFW пока не ладно
  }
}
 ...
 *Plus[]={ "<Enter> ","обновление изображения",
  ...
  First=true;
  StabWin.Draw();             // разметка экрана c прорисовкой проекции корпус
 int ans=_Enter;
...
MainLoop:
  switch( ans )
  { case _Esc: goto Ret; // StabWin.Close(); break; // неявный деструктор позже
