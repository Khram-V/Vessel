...
Plane& Plane::Set(_Real X,_Real Z,_Real _X,_Real _Z )// отступ от право-нижнего
       { aX=X,uX=_X; dx=( uX-aX )/( ux-ax );         // угла и размерения новой
...
Real Plane::Z( int z ){ return aZ + z*dz; }  // Нормальные координаты ViewPort
...
//
 const char *cU[4]={ " Кирпич:"," Эллипс:"," Шлюпка:"," Корпус:" },
            *Sc[2]={ "  по осадке ","  по борту  " };
 static Real cX=1.0,cY=1.0,cZ=1.0,cL=20.0,cB=10.0, // aФ, длина и ширина модели
             Lc=0.0,Ac=1.0, //-- прореживание по Lc[мм] при сломе на угол Ac[°]
             dX=0.0,dZ=0.0, //++ продольные и вертикальные смещения корпуса
             Sp=3.0;      // степень вогнутости скуловых обводов в оконечностях
 static int cM=1,cN=65,   // индекс и количество шпаций аналитической модели
            sc=1;  // индекс глубины скуловой вогнутости по осадке или по борту
 static Mlist Menu_p[]={ { 1,0,"Аналитические обводы корпуса" }     //  0
                       , { 2,28," 1 - на степенных функциях"  }     //! 1
                       , { 1,28," 2 - синусно-показательных"  }     //! 2
                    // , { 1,28," исправление модели штевней" }
                       , { 2,28," <<Простые преобразования>>" }     //! 3
                       , { 1,6, " x∙%-6.4lf",&cX }                  //  4
                       , { 0,6, " y∙%-6.4lf",&cY }                  //  5
                       , { 0,6, " z∙%-6.4lf",&cZ }                  //  6
                       , { 1,6, " Сдвиг по X:%6.3lf",&dX }          //  7
                       , { 0,6," Z:%6.3lf",&dZ }                    //  8
                       , { 1,6, " Рёбра ≥ %-6.1lf",&Lc }            //  9
                       , { 0,4, " мм на %-4.0lf",&Ac},{0,0,"°"}     //  10÷11
                       , { 2,28," <<Эллипсоиды и другое...>>" }     //! 12
                       , { 1,9, cU[cM] }                            //  13
                       , { 0,3, " L=%-3.0lf",&cL }                  //  14
                       , { 0,3, " B=%-3.0lf",&cB }                  //  15
                       , { 0,3, " N=%-3d",   &cN }                  //  16
                       , { 2,28," << обводы скуловых волн >>" }     //  17
                       , { 1,12,Sc[sc]},{0,3," степень %3.3lf",&Sp} // 18÷19
                       , { 1,0 } };                                   //  20
...
MainLoop: if( ans )Win.Draw(); Wid=false;        //  и - главный цикл запросов
  ...
  { case _F1: Hull_Help(); ans=0; break;
    ...
    case _F4: // if( Win.ScanStatus()&ALT )exit( 0 );     // поочередный запрос
          { TextMenu T( Mlist( Menu_p ),&Win,2 ); cX=cY=cZ=1.0; dX=dZ=0.0;
            while( (ans=T.Answer())>=3 )            // геометрического описания
            { Ac=remainder( Ac,360 );              // градусные угловые отсчёты
              if( ans==3  ){ Kh.Aphines( cX,cY,cZ,dX,dZ,Lc,Ac ); break; }
              if( ans==12 ){ Kh.ModelEx( cL,cB,cN,cM ); break; }
              if( ans==17 ){ Kh.BilgeEx( Draught*(sc+1),Sp ); break; } // скуловые обводы
              if( ans==13 )Menu_p[13].Msg=cU[++cM%=4];// ключи выбора модели
