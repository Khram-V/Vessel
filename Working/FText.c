...
{ static char *S; static int L=0; int c,k=0;       // Чтение текстовой строки
  ...
  while( !feof( F ) )
   if( (c=fgetc( F ))!='\r' )                      // конец строки может быть
...
{                                      //    для доступа к буферному массиву
 static char *OutBuf=NULL;             // собственно выходной буфер
 static DWORD OutLen=0,OutCur=0;       // полная неснижаемая длина буфера
  if( !l )OutCur=0; else OutCur+=l;    // текущее положение маркера заполнения
...
//
  char St[MAX_PATH];                        // строка кодировки для Far-manager
...
//    printf( "%12s %8ld Read %s ",St,
      ...
      { if( (Fi=fopen( FileData.cFileName,"rt" ))!=NULL )
        { int Len,m,e=0,t=abs( T );
          long ni=0,no=0; S=OutPut( 0 );        //! установка нового буфера
          while( !feof( Fi ) )
          { Len=strlen( inS=getString( Fi ) );
            if( !t ){ strcpy( S,inS ); l=Len; } // и для начала
            else for( l=k=0; k<Len; k++ )       // снимаются все табуляторы
            { if( (S[l]=inS[k])!=9 )l++; else do S[l]=' '; while( ++l%t );
            }                             //
            S[Len=l]=0;                   // Отсечение концевых пробелов
            s=S+Len-1;                    //
          ...
          } fclose( Fi );          S=OutPut( 0 ); // сброс буфера под запись
