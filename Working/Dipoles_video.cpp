...
//int t=Time_count;            // счетчик может нежданно поменяться - не должен
 ...
 Real rt=RealTime;
  ...
  Title( _Format( "Ч:%d [рис:%u/счет:%u]: T=%s = %1.2f%%    ==  %ld тики",nDip,
           vk,Time_count,DtoA(rt/3600000.0),rt*100.0/ElapsedTime(),RealTime ) );
  //
  ...
  { Dipole &D=Dp[i]; Vector P=D.R-Center;
    ...
    for( k=max( 0u,Time_count-Dipole_route+1 ); k<Time_count; k++ )
